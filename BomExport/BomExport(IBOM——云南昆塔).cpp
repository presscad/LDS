...
void AddPartTblSheet(CModelBOM &bom,LPDISPATCH pWorkSheet)
{
	...
	str_arr[0] = "段号"; str_arr[1] = "件号"; str_arr[2] = "材质"; str_arr[3] = "规格";
	str_arr[4] = "长度"; str_arr[5] = "数量"; str_arr[6] = "单重"; str_arr[7] = "总重"; 
	double col_arr[8] = { 10.0,10.5,11.5,11.5,
						  9.5,10.0,10.0,12.0};
	...
	for(SUPERLIST_NODE<BOMPART>*pNode=BomArr.GetFirst();pNode;pNode=BomArr.GetNext())
	{
		...
		if (sMaterial.EqualNoCase("Q345") || sMaterial.EqualNoCase("Q355"))
		...
		if(pBomPart->length>0)
		{
			if (pBomPart->cPartType == BOMPART::ACCESSORY&&pBomPart->siSubType == BOMPART::SUB_TYPE_STEEL_GRATING)
				map.SetValueAt(index, 4, COleVariant(CXhChar50("%.fX%.f", pBomPart->wide, pBomPart->length)));
			else
		...
		map.SetValueAt(index,7,COleVariant(CXhChar50("%.1f", pBomPart->fPieceWeight*pBomPart->GetPartNum())));
		index++;
