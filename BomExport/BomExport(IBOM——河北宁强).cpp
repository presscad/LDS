...
{	//V13 修复材质Q355输出错误问题 wht 19-08-25
	return 13;
}
#include "excel9.h"
#include "Bom.h"
...
static CXhChar16 GetMaterialStr(BOMPART* pBomPart,BOOL bIncQuality)
{
	...
	if (materialStr.GetLength() <= 0)
		materialStr.Copy(pBomPart->sMaterial);
...
{
	CXhChar16 sMat=bIncMat?GetMaterialStr(pBomPart,false):CXhChar16();
	CXhChar50 sSpec;
...
void AddPartTblSheet(CModelBOM &bom, LPDISPATCH pWorkSheet)
{
	...
	str_arr.SetSize(10);
	str_arr[0] = "段号"; str_arr[1] = "编号"; str_arr[2] = "材质"; str_arr[3] = "规格"; str_arr[4] = "长度"; 
	str_arr[5] = "宽度"; str_arr[6] = "厚度"; str_arr[7] = "件数"; str_arr[8] = "单重"; str_arr[9] = "总重";
	...
	for (SUPERLIST_NODE<BOMPART>*pNode = BomArr.GetFirst(); pNode; pNode = BomArr.GetNext())
	{
		...
		if (sMat.EqualNoCase("Q345") || sMat.EqualNoCase("Q355"))
			Q345RowIndexArr.append(index + 2);
		if (sMat.EqualNoCase("Q420"))
			Q420RowIndexArr.append(index + 2);
		CXhChar50 sSpec = GetSpecStr(pBomPart, FALSE,FALSE);
		...
		if(pBomPart->wide>0 && pBomPart->cPartType == BOMPART::PLATE)
			map.SetValueAt(index, 5, COleVariant(CXhChar50("%.f", pBomPart->wide)));
		if(pBomPart->thick>0)
			map.SetValueAt(index, 6, COleVariant(CXhChar50("%.f", pBomPart->thick)));
		...
		map.SetValueAt(index, 9, COleVariant(CXhChar50("%.2f", pBomPart->fSumWeight)));
		index++;
	}
	...
	CExcelOper::SetRangeHorizontalAlignment(excel_sheet, "C2", CXhChar16("C%d", index), COleVariant((long)2));
	for (int i = 0; i < Q345RowIndexArr.GetSize(); i++)
	{
		int rowIndex = Q345RowIndexArr[i];
		CXhChar16 curCell("C%d", rowIndex);
		...
		CExcelOper::SetRangeHorizontalAlignment(excel_sheet, curCell, NULL, COleVariant((long)3));
	}
