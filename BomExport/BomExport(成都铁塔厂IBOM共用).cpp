...
void AddPartSpecTblSheet(CModelBOM &bom,LPDISPATCH pWorkSheet)
{
	...
	//添加标题行
	CExcelOper::SetMainTitle(excel_sheet,"A1","S1","中电建成都铁塔有限公司",20,14,3,2,false);
	...
	str_arr[8]="组焊";str_arr[9]="孔数";str_arr[10]="弯曲";str_arr[11]="切角";
	...
	for(SUPERLIST_NODE<BOMPART>*pNode=BomArr.GetFirst();pNode;pNode=BomArr.GetNext())
	{
		...
		if(cMaterial!='S')
			map.SetValueAt(index,4,COleVariant(QuerySteelMatMark(cMaterial)));			//材质
		...
		if(pBomPart->cPartType==BOMPART::PLATE)	//钢板考虑净面积重量
		{
			...
			map.SetValueAt(index,17,COleVariant(CXhChar16("%.2f",fWeight)));	//单件重量
		}
		else
...
void AddMatEstimateTbl(CModelBOM &bom,LPDISPATCH pWorkSheet)
{
	...
	//添加标题行
	CExcelOper::SetMainTitle(excel_sheet,"A1","R1","中电建成都铁塔有限公司",20,14,3,2,false);
	...
	for(SUPERLIST_NODE<BOMPART>*pNode=BomArr.GetFirst();pNode;pNode=BomArr.GetNext())
	{
		...
		map.SetValueAt(index,1,COleVariant(pBomPart->GetPartNo(bom.m_biMatCharPosType,bom.m_cMatCharSeparator)));//零件编号
		map.SetValueAt(index,2,COleVariant(GetPartTypeName(pBomPart,TRUE)));			//材料名称
		map.SetValueAt(index,3,COleVariant(GetSpecStr(pBomPart,TRUE)));					//设计规格
		char cMaterial=GetPartMaterialChar(pBomPart);
		map.SetValueAt(index,4,COleVariant(QuerySteelMatMark(cMaterial)));				//材质
		if(cMaterial=='P')	//Q420字体进行加粗
