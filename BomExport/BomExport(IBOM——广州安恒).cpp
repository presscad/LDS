...
static CXhChar50 GetSpecStr(BOMPART* pBomPart,BOOL bNeedPlateW=FALSE,BOOL bIncMat=FALSE)
{
	...
	if(pBomPart->cPartType==BOMPART::ANGLE)	
	{
		...
		sSpec.Printf("%sφ%sX%s",(char*)sMat,(char*)CXhChar16(pBomPart->wide),(char*)CXhChar16(pBomPart->thick));
	...
	else if(pBomPart->cPartType==BOMPART::PLATE)
	{
		...
		{
			if(bNeedPlateW)
				sSpec.Printf("%s-%.fX%.f",(char*)sMat,pBomPart->thick,pBomPart->wide);
			else
...
void AddPartTblSheet(CModelBOM &bom,LPDISPATCH pWorkSheet)
{
	...
	str_arr[5]="数量";str_arr[6]="单重";str_arr[7]="合计重量";str_arr[8]="备注";
	...
	for(SUPERLIST_NODE<BOMPART>*pNode=BomArr.GetFirst();pNode;pNode=BomArr.GetNext())
	{
		...
		map.SetValueAt(index,7,COleVariant(CXhChar50("%.2f",pBomPart->GetPartNum()*pBomPart->fPieceWeight)));
		index++;
