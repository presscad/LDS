...
void AddPartTblSheet(CModelBOM &bom,LPDISPATCH pWorkSheet)
{
	...
	str_arr[0] = "段号(只读)"; str_arr[1] = "部件号"; str_arr[2] = "规格";
	str_arr[3] = "长度(mm)"; str_arr[4] = "材质"; str_arr[5] = "单位"; str_arr[6] = "单基数量";
	str_arr[7] = "加工数量"; str_arr[8] = "单件重量"; str_arr[9] = "备注"; str_arr[10] = "单件孔数";
	double col_arr[11] = { 12.0,10.5,11.5,11.5,9.5,
						   6.0,12.0,12.0,12.5,38.0,9.5 };
	...
	for(SUPERLIST_NODE<BOMPART>*pNode=BomArr.GetFirst();pNode;pNode=BomArr.GetNext())
	{
		...
		map.SetValueAt(index, 4, COleVariant(sMaterial));
		if (sMaterial.EqualNoCase("Q345"))
			Q345RowIndexArr.append(index + 2);
		if (sMaterial.EqualNoCase("Q420"))
			Q420RowIndexArr.append(index + 2);
		if(pBomPart->cPartType==BOMPART::ANGLE)
			map.SetValueAt(index, 5, COleVariant("根"));
		else if (pBomPart->cPartType == BOMPART::PLATE)
		{
			if(sSpec.GetLength()>0 && sSpec.At(0)=='-')
				map.SetValueAt(index, 5, COleVariant("块"));
		}
	...
	CXhChar16 cell_end("K%d", index+1);
	...
	CExcelOper::SetRangeHorizontalAlignment(excel_sheet, "E2", CXhChar16("E%d",index), COleVariant((long)2));
	for (int i = 0; i < Q345RowIndexArr.GetSize(); i++)
	{
		int rowIndex = Q345RowIndexArr[i];
		CXhChar16 curCell("E%d", rowIndex);
