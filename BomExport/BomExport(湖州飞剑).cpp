...
static short GetKaiHeJiaoN(BOMPART *pPart)
{
	...
	//1～440mm长记为1， 441～540记为2， 541～1000记为3，1001～1500记为4，1501～2000记为5，2001～2500记为6，2501～3000记为7； 湖州飞剑新标准 wht 15-12-25
	//10001以上均记为4 湖州飞剑新标准 wht 17-02-24
...
{	
	...
	for(SUPERLIST_NODE<BOMPART> *pNode=bom.listParts.EnumFirst();pNode;pNode=bom.listParts.EnumNext())
	{
		...
		if(pPart->cPartType==BOMPART::ANGLE)
		{
			...
			long nKaiHeJiaoN=GetKaiHeJiaoN(pAngle);
...
void AddPartSpecTblSheet(CModelBOM &bom,LPDISPATCH pWorksheet)
{
	...
	for(SUPERLIST_NODE<BOMPART> *pNode=bom.listParts.EnumFirst();pNode;pNode=bom.listParts.EnumNext())
	{
		...
		map.SetValueAt(index,2,COleVariant(pPart->GetPartNo(bom.m_biMatCharPosType,bom.m_cMatCharSeparator)));			//编号
		map.SetValueAt(index,3,COleVariant(GetPartTypeName(pPart)));				//材料
