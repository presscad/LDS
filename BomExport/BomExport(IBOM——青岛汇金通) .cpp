...
void AddPartTblSheet(CModelBOM &bom,LPDISPATCH pWorkSheet)
{
	...
	str_arr[0]="段号";str_arr[1]="编号";str_arr[2]="规格";
	str_arr[3]="长度";str_arr[4]="数量";str_arr[5]="单重";
	str_arr[6]="合计重量";str_arr[7]="备注";
	CExcelOper::AddRowToExcelSheet(excel_sheet,1,str_arr,NULL,TRUE);
	int index=0;
	...
	for(SUPERLIST_NODE<BOMPART>*pNode=BomArr.GetFirst();pNode;pNode=BomArr.GetNext())
	{
		...
		map.SetValueAt(index,6,COleVariant(CXhChar50("=E%d*F%d",index+2,index+2)));
		map.SetValueAt(index,7,COleVariant(pBomPart->sNotes));
		index++;
