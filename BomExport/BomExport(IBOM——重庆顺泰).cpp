...
CXhChar100 GetBomPartSpec(BOMPART *pBomPart)
{
	...
	{
		if (materialStr.GetLength() <= 0 && pBomPart->sMaterial.GetLength() > 0)
			materialStr.Copy(pBomPart->sMaterial);	//兼容Q355 wht 19-08-29
...
void AddPartTblSheet(CModelBOM &bom,LPDISPATCH pWorkSheet)
{
	...
	CExcelOper::SetRangeColWidth(excel_sheet,15.0,"C1");
	//清空原有内容
