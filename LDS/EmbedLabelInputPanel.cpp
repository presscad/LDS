...
CEmbedLabelInputPanel::PARTINFO CEmbedLabelInputPanel::GetPartInfo(CLDSPart* pPart)
{
	...
	if(pCurrPart->IsAngle())
	{
		...
		if(partinfo.ciWorkWing=='X')
			partinfo.szAnchorWing="X肢定位";
		else if(partinfo.ciWorkWing=='Y')
			partinfo.szAnchorWing="Y肢定位";
		else if(partinfo.ciWorkWing=='A')
			partinfo.szAnchorWing="双肢定位";
		else
			partinfo.szAnchorWing="智能定位";
...
void CEmbedLabelInputPanel::UpdatePartInfoToUI(PARTINFO* pPartInfo,bool blFrontTrueSideFalse/*=true*/)
{
	...
	else
	{
		...
		{
			GetDlgItem(IDC_S_PRIOR_WING_G2)->SetWindowText("X肢心距：");
			GetDlgItem(IDC_S_SLAVE_WING_G2)->SetWindowText("Y肢心距：");
		}
		else
		{
			GetDlgItem(IDC_S_PRIOR_WING_G2)->SetWindowText("工作肢心距：");
			GetDlgItem(IDC_S_SLAVE_WING_G2)->SetWindowText("另一肢心距：");
		}
	}
}
...
bool CEmbedLabelInputPanel::ConfirmAndNextLabelGroup()
{
	...
	{
		MessageBox("编号录入结束!",theApp.m_pszAppName);//存在一些未知异常导致AfxGetAppName错误，故在此指定AppName wjh-2019.6.23
		CDialog::OnOK();
		return true;
	}
	else
...
void CEmbedLabelInputPanel::OnChangeEOriginPartNo()
{
	...
	if(m_bSyncRotationParts&&m_pMirRotPart)
	{
		...
		if(strlen(m_pMirRotPart->Label)==0)
			sMsgTitle.Append(", 侧面录入新编号");//m_sErrorMsg
		else if(m_sMirRotPartNo.CompareNoCase(m_pMirRotPart->Label)==0)
			sMsgTitle.Append(", 侧面保持原编号");//m_sErrorMsg
		else
			sMsgTitle.Append(", 侧面更改原编号");//m_sErrorMsg
...
void CEmbedLabelInputPanel::UpdateMirRotPartInfoByLabel()
{
	...
	if(m_pMirRotPart&&strlen(m_pMirRotPart->Label)==0)
		strcpy(sSideMsgTitle," 侧面录入新编号");//m_sErrorMsg
	else if(m_pMirRotPart&&m_sMirRotPartNo.CompareNoCase(m_pMirRotPart->Label)==0)
		strcpy(sSideMsgTitle," 侧面保持原编号");//m_sErrorMsg
	else
		strcpy(sSideMsgTitle," 侧面更改原编号");//m_sErrorMsg

	m_ctrlErrorMsg.GetWindowTextA(sMsgTitle,50);
	char* pszSideTitle=strstr(sMsgTitle,"侧面");
	if(pszSideTitle)
	...
	{
		//TODO:m_pCurrPart是否需要更新为新变量，还有待商榷
		if(strlen(pCurrMirRotPart->Label)==0)
			m_ctrlErrorMsg.SetWindowText("侧面录入新编号");//m_sErrorMsg
		else if(m_sMirRotPartNo.CompareNoCase(pCurrMirRotPart->Label)==0)
			m_ctrlErrorMsg.SetWindowText("侧面保持原编号");//m_sErrorMsg
		else
			m_ctrlErrorMsg.SetWindowText("侧面更改原编号");//m_sErrorMsg
		m_ciErrorMsgMode=2;
	}
#endif
	if(m_pMirXnRotPart)
