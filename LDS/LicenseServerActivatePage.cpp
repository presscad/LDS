...
void CLicenseServerActivatePage::OnBnCreateIdentityFile()
{
	...
	if(PRODUCT_TYPE==PRODUCT_TMA)	
		sprintf(fn,"TMA%d.apl",uDogSerial);
	else if(PRODUCT_TYPE==PRODUCT_LMA)
		sprintf(fn,"LMA%d.apl",uDogSerial);
	else if(PRODUCT_TYPE==PRODUCT_TSA)
		sprintf(fn,"TSA%d.apl",uDogSerial);
	else if(PRODUCT_TYPE==PRODUCT_LDS)
		sprintf(fn,"LDS%d.apl",uDogSerial);
	else if(PRODUCT_TYPE==PRODUCT_TDA)
		sprintf(fn,"TDA%d.apl",uDogSerial);
	/*else if(v2c.cLicProductType==PRODUCT_CTMA)
		...
		sprintf(fn,"TAP%d.apl",uDogSerial);
	else if(PRODUCT_TYPE==PRODUCT_PNC)
		sprintf(fn,"PNC%d.apl",uDogSerial);
	else if(PRODUCT_TYPE==PRODUCT_IBOM)
		sprintf(fn,"IBOM%d.apl",uDogSerial);
...
void CLicenseServerActivatePage::OnBnImportAuthorizeFile()
{
	...
	if(wContentsLen>0)
	{
		CXhChar200 svcfile("%s%s.svc",theApp.execute_path,(char*)productname);
