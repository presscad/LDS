...
BOOL CLicenseAuthorizeDlg::OnInitDialog()
{
	...
	UINT siPortNumber=theApp.GetProfileInt("Settings","master_host_port_number",0);
	if(siPortNumber>0)
...
#endif
	DWORD retCode=ImportLicFile(licfile,cProductType,version);	
...
void CLicenseAuthorizeDlg::OnBnApplyForLanLicense()
{
	...
	theApp.WriteProfileInt("Settings","master_host_port_number",m_siPortNumber);
	...
	if(sError.GetLength()>0)
	{
		...
		else //if(commCore.IsSocketConnect())
			sError.Append(CXhChar100(". 核心通信端口{%s:%d}连接失败!",(char*)sHostIpAddress,m_siPortNumber));
		AfxMessageBox(sError);
		return;
	}
	CXhChar16 szSubKey="product";
	if(PRODUCT_TYPE==PRODUCT_TMA||PRODUCT_TYPE==PRODUCT_CTMA)
		szSubKey.Copy("TMA");
