...
{
	ClearTmplParams();
	name.Copy("EB型单挂点直线塔导线V串挂线(方形横担)");
	...
	pParaAngle->desEnd.szXorCtrlWingOffset = "-$2#T";	//表示负角钢2的肢厚
	pParaAngle->desEnd.szYorOthrWingOffset[0] = '0';
	pParaRod = listParaRods.Add(5);//横担顶点上杆件
	pParaRod->idStartNode = N20_ID;	//1;
...
{
	ClearTmplParams();
	name.Copy("EB型单挂点直线塔导线V串挂线(上相导线V串内侧挂点)");
