...
//初始化模板参数
//IStdComTempl::STDTMPL_STRU_META_SECT_BTM_1_GD
void CStdComTemplWireStructure::InitComTempl_META_SECT_BTM_1_GD()
{
	ClearTmplParams();
	name.Copy("耐张塔塔横隔材底面单GD挂点");
	szSketchImgFile.Copy("JC-META-GD2.bmp");
	...
	pParam->sNotes.Copy("挂线角钢间隙间隙, GD挂点金具宽度+间隙，mm");
	...
	const int ANG_BTM_L = 2;	//底部外贴T型挂线短角钢
	const int ANG_BTM_R = 3;	//底部外贴T型挂线短角钢
	const int ANG_FRT = 4;		//正面外贴短角钢
	pParaAngle = (CStdTemplParaAngle*)listParaRods.Add(ANG_BTM_L, CLS_LINEANGLE);
	...
	//
	pParaAngle = (CStdTemplParaAngle*)listParaRods.Add(ANG_BTM_R, CLS_LINEANGLE);
	pParaAngle->cbMirCreateFlag = 0X01;
	pParaAngle->idStartNode = N10_ID;
	pParaAngle->ciSectStyle = 'L';
	pParaAngle->blShortAngle = true;
	...
	//
	pParaAngle = (CStdTemplParaAngle*)listParaRods.Add(ANG_FRT, CLS_LINEANGLE);
	pParaAngle->cbMirCreateFlag = 0X01;
	pParaAngle->idStartNode = N10_ID;
	pParaAngle->ciSectStyle = 'L';
	pParaAngle->attachangle.idBaseAngle = 1;		//前侧横担主材角钢
	...
	pParaPlate->connrods[1].InitBasicInfo(ANG_FRT, true, 1, 2, true);
...
//翻译模板设计参数
extern CTower Ta;
bool CStdComTemplWireStructure::TransTemlInstance_SECT_BTM_1_GD()
{
