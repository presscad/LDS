...
DWORD CSnapTypeVerify::AddVerifyType(int provider,int iObjClsType)
{
	if(provider==1)
	{
		...
		if(iObjClsType==IDbEntity::DbPoint)	// = 1;
			dwFlag=GetSingleWord(IDbEntity::DbPoint);
		else if(iObjClsType==IDbEntity::DbLine)		// = 2;
			dwFlag=GetSingleWord(IDbEntity::DbLine);
		else if(iObjClsType==IDbEntity::DbArcline)	// = 3;
			dwFlag=GetSingleWord(IDbEntity::DbArcline);
		else if(iObjClsType==IDbEntity::DbRect)	// = 4;
			dwFlag=GetSingleWord(IDbEntity::DbRect);
		else if(iObjClsType==IDbEntity::DbArrow)	// = 5;
			dwFlag=GetSingleWord(IDbEntity::DbArrow);
		else if(iObjClsType==IDbEntity::DbMark)		// = 6;
			dwFlag=GetSingleWord(IDbEntity::DbMark);
		else if(iObjClsType==IDbEntity::DbPolyline)	// = 7;
			dwFlag=GetSingleWord(IDbEntity::DbPolyline);
		else if(iObjClsType==IDbEntity::DbCircle)	// = 8;
			dwFlag=GetSingleWord(IDbEntity::DbCircle);
		else if(iObjClsType==IDbEntity::DbSpline)	// = 9;
			dwFlag=GetSingleWord(IDbEntity::DbSpline);
		else if(iObjClsType==IDbEntity::DbText)		// =10;
			dwFlag=GetSingleWord(IDbEntity::DbText);
		else if(iObjClsType==IDbEntity::DbMText)	// =11;
			dwFlag=GetSingleWord(IDbEntity::DbMText);
		else if(iObjClsType==IDbEntity::DbDiametricDimension)	// =12;
			dwFlag=GetSingleWord(IDbEntity::DbDiametricDimension);
		else if(iObjClsType==IDbEntity::DbRadialDimension)		// =13;
			dwFlag=GetSingleWord(IDbEntity::DbRadialDimension);
		else if(iObjClsType==IDbEntity::DbAngularDimension)		// =14;
			dwFlag=GetSingleWord(IDbEntity::DbAngularDimension);
		else if(iObjClsType==IDbEntity::DbAlignedDimension)		// =15;
			dwFlag=GetSingleWord(IDbEntity::DbAlignedDimension);
		else if(iObjClsType==IDbEntity::DbRotatedDimension)		// =16;
			dwFlag=GetSingleWord(IDbEntity::DbRotatedDimension);
		m_dwDrawingSpaceFlag|=dwFlag;	//provider=1
		return m_dwDrawingSpaceFlag;
	}
	else if(provider==2)
...
bool CSnapTypeVerify::IsValidSolidObjType(int iObjClsType){
	...
	case CLS_HOLEWALL:
		return (m_dwSolidSpaceFlag&GetSingleWord(SELECTINDEX_HOLEWALL))>0;
	case CLS_FITTINGS:
		return (m_dwSolidSpaceFlag&GetSingleWord(SELECTINDEX_FITTINGS))>0;
	case CLS_SPHERE:
...
bool SELOBJ::TranslateHoleWall(ISolidDraw* pSolidDraw)
{
	if(provider!=2||this->iObjType!=CLS_HOLEWALL||this->hObj>=0||pSolidDraw==NULL)
		return false;
	OBJECT_HANDLER handler;
	if(!pSolidDraw->GetSolidObject((DWORD)this->hObj,&handler)||handler.hiberid.HiberUpId(1)!=1)
		return false;
	this->hObj=handler.hiberid.masterId;
	this->hResidePart=handler.hiberid.HiberUpId(2);
	return true;
}
