...
struct FilePos
{
	...
	bool FileReadText( MCD_STR& strDoc,char *pStr = NULL);
	bool FileCheckRaggedEnd( void* pBuffer,char *pStr=NULL);
	bool FileReadNextBuffer();
	bool FileWrite( void* pBuffer, const void* pConstBuffer = NULL );
	bool FileWriteText( const MCD_STR& strDoc, int nWriteStrLen = -1,char *pStr = NULL);
...
bool FilePos::FileCheckRaggedEnd( void* pBuffer ,char *pStr)
{
...
bool FilePos::FileReadText( MCD_STR& strDoc,char *pStr /*=NULL*/)
{
	bool bSuccess = true;
	MCD_STRCLEAR( m_strIOResult );
	if ( ! m_nOpFileByteLen )
	...
	if ( (MCD_INTFILEOFFSET)m_nOpFileByteLen >= nBytesRemaining && pStr==NULL)
	{
	...
	else // single or multibyte file (i.e. not UTF-16)
	{
		...
		if ( TextEncoding::CanConvert(MCD_ENC,m_strEncoding) )
		{
			...
			if (pStr!=NULL)
				...
				bSuccess = FileRead( pBuffer );
			if ( (bSuccess && bCheckRaggedEnd) && pStr==NULL)
				FileCheckRaggedEnd( (void*)pBuffer ,pStr);
			TextEncoding textencoding( m_strEncoding, (const void*)pBuffer, m_nOpFileTextLen );
			int nMBLen = textencoding.PerformConversion( NULL, MCD_ENC );
			int nBufferSizeForGrow = nMBLen + nMBLen/100; // extra 1%
			MCD_CHAR* pMBBuffer = MCD_GETBUFFER(strDoc,nBufferSizeForGrow);
...
bool FilePos::FileWriteText( const MCD_STR& strDoc, int nWriteStrLen/*=-1*/,char *pStr /*=NULL*/)
{
	bool bSuccess = true;
	MCD_STRCLEAR( m_strIOResult );
...
bool CMarkup::Load( MCD_CSTR_FILENAME szFileName ,char *pStr)
{
	if ( m_nDocFlags & (MDF_READFILE|MDF_WRITEFILE) )
		return false;
	if ( ! ReadTextFile(szFileName, m_strDoc, &m_strResult, &m_nDocFlags,NULL,pStr) )
		return false;
	return x_ParseDoc();
}

bool CMarkup::ReadTextFile( MCD_CSTR_FILENAME szFileName, MCD_STR& strDoc, MCD_STR* pstrResult, 
						   int* pnDocFlags, MCD_STR* pstrEncoding ,char *pStr)
{
	// Static utility method to load text file into strDoc
	//
	FilePos file;
	file.m_nDocFlags = (pnDocFlags?*pnDocFlags:0) | MDF_READFILE;
	//如果存在pStr则从pStr中拷贝字符串
	if (pStr!=NULL)
	{
		file.m_strEncoding = MCD_T("UTF-8");
		file.m_nOpFileTextLen = (int)strlen(pStr);
		//得到当前文本长度
		TextEncoding textencoding( file.m_strEncoding, (const void*)pStr, file.m_nOpFileTextLen );
		int nMBLen = textencoding.PerformConversion( NULL, MCD_ENC );
		file.m_nOpFileTextLen = nMBLen;
		file.m_nOpFileByteLen = nMBLen;
		file.m_nFileByteLen = nMBLen;
		//去除冗余结尾
		file.FileCheckRaggedEnd( (void*)pStr ,pStr);
		file.FileReadText( strDoc ,pStr);
		return TRUE;
	}
...
bool CMarkup::Save( MCD_CSTR_FILENAME szFileName,char *pStr)
{
	if ( m_nDocFlags & (MDF_READFILE|MDF_WRITEFILE) )
		return false;
	return WriteTextFile( szFileName, m_strDoc, &m_strResult, &m_nDocFlags ,NULL,pStr);
}

bool CMarkup::WriteTextFile( MCD_CSTR_FILENAME szFileName, const MCD_STR& strDoc, MCD_STR* pstrResult, int* pnDocFlags, MCD_STR* pstrEncoding ,char *pStr)
{
	// Static utility method to save strDoc to text file
	//
	FilePos file;
	file.m_nDocFlags = (pnDocFlags?*pnDocFlags:0) | MDF_WRITEFILE;
	//如果存在pStr则将xml中的字符串导出到pStr中
	if (pStr!=NULL)
	{
		file.m_strEncoding = MCD_T("UTF-8");
		file.FileWriteText( strDoc ,-1,pStr);
		return TRUE;
	}
	bool bSuccess = file.FileOpen( szFileName );
	if ( pstrResult )
