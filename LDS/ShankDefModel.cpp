...
//////////////////////////////////////////////////////////////////////
HEIGHT_LEGS::HEIGHT_LEGS()
{
	idBodyNo=0;
	pModule = NULL;
	cnPresetLegCount=0;
	wnLevelHeightDiff=1000;
}
//////////////////////////////////////////////////////////////////////
// 局部函数
//////////////////////////////////////////////////////////////////////
	...
	if(pStartLinePart->GetClassTypeId()==CLS_LINEANGLE)
	{
		...
		{	
			...
			{	//不是终端分段杆件或当前坡度下面还接有其它坡度
				...
				else	//1->4：根据刘伟说明塔厂建模，变坡上段主材末端改为向楞线平推更合适 wjh-2019.12.03
					pLinePart.LineAnglePointer()->desEndPos.spatialOperationStyle=4;
			...
			{
				char cFaceOffsetDatumLine=0;
				if(pLinePart->pStart->layer(2)=='1'||pLinePart->pStart->layer(2)=='4')
					cFaceOffsetDatumLine='X';
				else if(pLinePart->pStart->layer(2)=='2'||pLinePart->pStart->layer(2)=='3')
				...
				//根据刘伟反馈以及比对过往旧模式放样模型塔身创建过程，变坡段顶部角钢塔身控制点平推，最好设为以X/Y肢心线为平推基准
				//如此对于核对角钢正负头更方便 wjh-2019.12.3
				pLinePart.LineAnglePointer()->desStartPos.cFaceOffsetDatumLine=cFaceOffsetDatumLine;
					 ...
					 SECTION_NODEARR& slopeStartNodeArr,SECTION_NODEARR slopeEndNodeArr,int nPoly,int idLinePartClsType)
{
	...
	//系统默认存在的呼高组模型,通过GetActiveModule()获取初始呼高，不稳定且限制了m_hActiveModule账值　wjh-2014.4.19
	if(!m_pDefine->mode.blAppendMode&&pModuleDef->IsFirstModule())
					 ...
					 SECTION_NODEARR& slopeStartNodeArr,SECTION_NODEARR slopeEndNodeArr,int nPoly,int idLinePartClsType)
{
	...
	{	//生成分段节点以及杆件
		...
		if (nPoly == 4 && linePartArr[i]->IsAngle())
		{	//[0]->Quad2;[1]->Quad1;[2]->Quad3;[3]->Quad4
			linePartArr[i].pAngle->m_cPosCtrlDatumLineType = (i==1||i==3)?'X':'Y';
			linePartArr[i].pAngle->m_cPosCtrlType = 1;	//1.X-Y平面平推
		}
	}
				...
				SECTION_NODEARR& slopeStartNodeArr,SECTION_NODEARR slopeEndNodeArr,int nPoly,int idLinePartClsType)
{
	...
	for(j=0;j<nPoly;j++)
	{
		...
		if(facepanel.ReadFromLibFile(lib_file,layout))
		{
			...
			keyNodeArr[3]=NULL;
			panelinfo.blFlipHorizon=true;	//原始模板一般以第一象限正面为基准定义，故左侧需进行肢朝向翻转	wjh-2019.4.24
			...
			panelinfo.blFlipHorizon=false;	//原始模板一般以第一象限正面为基准定义，故左侧需进行肢朝向翻转	wjh-2019.4.24
			operation.CreateStdTemplate(&facepanel,keyNodeArr,0,0,&panelinfo,&bodyword,&legword);
		}
	}
	//所有类型塔都支持V型标准面输入
...
CLDSNode* CShankDefineToTowerModel::AddDivScaleNode(CLDSNode *pAttachStartNode,CLDSNode *pAttachEndNode,double scale)
{
	...
	CFGWORD *pBodyCfgWord=NULL,bodycfgword;
	CFGWORD * pLegCfgWord=NULL,legcfgword;
	if(pAttachStartNode->IsLegObj())
		legcfgword=pAttachStartNode->UnifiedCfgword();
	else
		bodycfgword=pAttachStartNode->UnifiedCfgword();;
	if(pAttachEndNode->IsLegObj())
		legcfgword&=pAttachEndNode->UnifiedCfgword();
	else
		bodycfgword&=pAttachEndNode->UnifiedCfgword();;
	if(!bodycfgword.IsNull())
		pBodyCfgWord=&bodycfgword;
	if(!legcfgword.IsNull())
		pLegCfgWord=&legcfgword;
	CLDSNode* pNode=m_pTower->CheckOverlappedNode(pos,NULL,pBodyCfgWord,pLegCfgWord);
	if(pNode==NULL)
	{
		pNode=m_pTower->Node.append();
...
#ifdef __ANGLE_PART_INC_//defined(__LMA_)||defined(__LDS_)
...
bool CShankDefineToTowerModel::UpdateTowerModel(CTower* pTower,CShankDefine *pDefine,int nPoly,long idClassType)
{
	m_pDefine=pDefine;
	m_pTower=pTower;
	BYTE ciInitHeightSeial = 1;
	if (pDefine->mode.blAppendMode)
	{
		ciInitHeightSeial = pDefine->mode.ciInitHeightSerial;
		if (pTower != pDefine->mode.xarrTopNodesPtr[0]->BelongModel())
		{	//当前的模型空间为临时模拟空间，需将原始模型空间的借用点复制到临时模拟空间，以便显示 
			CLDSNode* pNode;
			long i,liMaxHandle=0x20;
			for (i = 0;i < 4;i++)
			{
				pNode = pTower->AppendNode(FALSE);
				pNode->handle = pDefine->mode.xarrTopNodesPtr[i]->handle;
				pNode->SetPosition(pDefine->mode.xarrTopNodesPtr[i]->xOriginalPos);
				pNode->CopyProperty(pDefine->mode.xarrTopNodesPtr[i]);
				pNode->m_cPosCalType = 0;	//现有模型节点，可看作控制点，不考虑与其它节点间的依赖关系
				liMaxHandle = max(liMaxHandle, pNode->handle);
			}
			for (i = 0;i < 4;i++)
			{
				pNode = pTower->AppendNode(FALSE);
				pNode->handle = pDefine->mode.xarrBtmNodesPtr[i]->handle;
				pNode->SetPosition(pDefine->mode.xarrBtmNodesPtr[i]->xOriginalPos);
				pNode->CopyProperty(pDefine->mode.xarrBtmNodesPtr[i]);
				pNode->m_cPosCalType = 0;	//现有模型节点，可看作控制点，不考虑与其它节点间的依赖关系
				liMaxHandle = max(liMaxHandle, pNode->handle);
			}
			pTower->handle = liMaxHandle + 1;
		}
	}
	else
	{	//增量模式下不能清空现有塔模型
	...
	if(pDefine->SlopeNum()<1)
		return false;
	pDefine->AllocateCfgWord(ciInitHeightSeial);
	dims.Empty();
	SECTION_NODEARR slopeStartNodeArr;
	memset(&slopeStartNodeArr, 0, sizeof(SECTION_NODEARR));
	int liInitSlopeIndex = 0;
	if (pDefine->mode.blAppendMode)
	{
		liInitSlopeIndex = 1;
		memcpy(slopeStartNodeArr.nodePtrArr, pDefine->mode.xarrTopNodesPtr, sizeof(CLDSNodePtr) * 4);
		//AddSlope中节点的象限排序由于历史原因，与实际塔模型1，2号调换 wjh-2019.5.23
		slopeStartNodeArr.nodePtrArr[0] = pDefine->mode.xarrTopNodesPtr[1];
		slopeStartNodeArr.nodePtrArr[1] = pDefine->mode.xarrTopNodesPtr[0];
	}
	for(int i= liInitSlopeIndex; i<=pDefine->SlopeNum();i++)
	{
...
bool CShankDefineToTowerModel::AddSlopeToTowerModel(SECTION_NODEARR &slopeStartNodeArr,int nPoly,int iSlope,int nSlopeNum,int idClassType)
{
	...
	if(iSlope>0)
	{
		...
		slopePosInfo.coordBottomZ= m_pDefine->SlopeBottomCoordZ(iSlope);
		...
		CShankLeg* pDatumLeg= slopePosInfo.pDatumLeg = pShankSlope->GetDatumLeg();
		if(pDatumLeg)	//不在此赋值将导致基准腿的基础节点与坡段底部节点重复 wjh-2013.6.5
			pDatumLeg->m_bDatumLeg=TRUE;
		if (slopePosInfo.bBottomSlope&&pShankSlope->ModuleNum() + pShankSlope->SegmentNum() == 0)
			return false;
		...
		{	//调整多呼高多接腿时各接腿主材基础端的位置设计参数
			...
			for(pLegAngle=(CLDSLineAngle*)m_pTower->Parts.GetFirst(CLS_LINEANGLE);pLegAngle;pLegAngle=(CLDSLineAngle*)m_pTower->Parts.GetNext(CLS_LINEANGLE))
			{
				if(pLegAngle->layer(0)=='L'&&pLegAngle->layer(1)=='Z'&&isdigit(pLegAngle->layer(2)))
				{
					...
					{
						//1->4：根据刘伟说明塔厂建模改为向楞线平推更合适 wjh-2019.12.03
						pLegAngle->desEndPos.spatialOperationStyle=4;
...
bool CShankDefineToTowerModel::BuildLegacyFrameTowerModel(CTower* pTower,CFrameDefine *pDefine,int nPoly/*=4*/)
{
	m_pTower=pTower;
	m_pTower->Empty();
	m_pTower->InitTower();
	m_pTower->InitModule();
	CLDSNode *pVertexNodeArr[8]={NULL},*pPrevVertexNodeArr[8]={NULL};
	CSuperSmartPtr<CLDSLinePart>linePartArr[8];
	for(int i=0;i<pDefine->m_xOpenArr.GetSize();i++)
	{
		CFrameDefine::OPEN_SIZE xSize=pDefine->m_xOpenArr[i];
		//计算节点位置
		if(nPoly==3)
		{
			//根据第一次输入的正/侧面开口的值以及以后输入的正面开口计算相应的侧面开口的值
			if(i!=0)
				xSize.m_fSW=xSize.m_fFW*(pDefine->m_xOpenArr[0].m_fSW/pDefine->m_xOpenArr[0].m_fFW);
			double fX=xSize.m_fFW/2;
			double fY=sqrt(xSize.m_fSW*xSize.m_fSW-xSize.m_fFW*xSize.m_fFW*0.25)/3;
			double fZ=xSize.m_fVH;
			pVertexNodeArr[0]=m_pTower->AppendNode();	//添加结点
			pVertexNodeArr[0]->SetLayer("SP1");			//设置图层名
			pVertexNodeArr[0]->SetPosition(-fX, fY, fZ);
			pVertexNodeArr[1]=m_pTower->AppendNode();
			pVertexNodeArr[1]->SetLayer("SP2");
			pVertexNodeArr[1]->SetPosition(fX, fY, fZ);
			pVertexNodeArr[2]=m_pTower->AppendNode();
			pVertexNodeArr[2]->SetLayer("SP3");
			pVertexNodeArr[2] ->SetPosition(0, -2*fY, fZ);
		}
		else if(nPoly==4)
		{
			double fX=xSize.m_fFW/2;
			double fY=xSize.m_fSW/2;
			double fZ=xSize.m_fVH;
			//添加结点并设置结点坐标以及图层名
			pVertexNodeArr[0]=m_pTower->AppendNode();
			pVertexNodeArr[0]->SetLayer("SP2");
			pVertexNodeArr[0]->SetPosition(-fX, fY, fZ);
			pVertexNodeArr[1]=m_pTower->AppendNode();
			pVertexNodeArr[1]->SetLayer("SP1");
			...
			pVertexNodeArr[2]->SetLayer("SP3");
			pVertexNodeArr[2]->SetPosition(fX, -fY, fZ);
			pVertexNodeArr[3]=m_pTower->AppendNode();
			pVertexNodeArr[3]->SetLayer("SP4");
			pVertexNodeArr[3]->SetPosition(-fX, -fY, fZ);
		}
		else if(nPoly==5)
		{
			double fX =xSize.m_fFW/2;
			double fY =xSize.m_fFW*sin(72.0/180*Pi);
			double fY1=xSize.m_fFW*cos(72.0/180*Pi);
			double fR =fX/cos(54.0/180*Pi);
			double fR1=fX*tan(54.0/180*Pi);
			double fZ =xSize.m_fVH;
			//添加结点并设置结点坐标以及图层名
			pVertexNodeArr[0]=m_pTower->AppendNode();
			pVertexNodeArr[0]->SetLayer("SP1");
			...
			pVertexNodeArr[3]->SetLayer("SP4");
			pVertexNodeArr[3]->SetPosition(0,-fR,fZ);
			pVertexNodeArr[4]=m_pTower->AppendNode();
			pVertexNodeArr[4]->SetLayer("SP5");
			pVertexNodeArr[4]->SetPosition(-fX-fY1,fR1-fY,fZ);
		}
		else if(nPoly==6)
		{
			double fX=xSize.m_fFW;
			double fY=(sqrt(3.0)/2)*fX;
			double fZ=xSize.m_fVH;
			//添加结点并设置结点坐标以及图层名
			pVertexNodeArr[0]=m_pTower->AppendNode();
			pVertexNodeArr[0]->SetLayer("SP1");
			pVertexNodeArr[0]->SetPosition(-fX,0,fZ);
			pVertexNodeArr[1]=m_pTower->AppendNode();
			pVertexNodeArr[1]->SetLayer("SP2");
			pVertexNodeArr[1]->SetPosition(-fX/2,fY,fZ);
			pVertexNodeArr[2]=m_pTower->AppendNode();
			pVertexNodeArr[2]->SetLayer("SP3");
			pVertexNodeArr[2]->SetPosition(fX/2,fY,fZ);
			pVertexNodeArr[3]=m_pTower->AppendNode();
			pVertexNodeArr[3]->SetLayer("SP4");
			pVertexNodeArr[3]->SetPosition(fX,0,fZ);
			pVertexNodeArr[4]=m_pTower->AppendNode();
			pVertexNodeArr[4]->SetLayer("SP5");
			pVertexNodeArr[4]->SetPosition(fX/2,-fY,fZ);
			pVertexNodeArr[5]=m_pTower->AppendNode();
			pVertexNodeArr[5]->SetLayer("SP6");
			pVertexNodeArr[5]->SetPosition(-fX/2,-fY,fZ);
		}
		else if(nPoly==8)
		{
			double fX=xSize.m_fFW*0.5;
			double fY=(1.0/sqrt(2.0))*xSize.m_fFW;
			double fZ=xSize.m_fVH;
			//添加结点并设置结点坐标以及图层名
			pVertexNodeArr[0]=m_pTower->AppendNode();
			pVertexNodeArr[0]->SetLayer("SP1");
			...
			pVertexNodeArr[5]->SetLayer("SP6");
			pVertexNodeArr[5]->SetPosition(fX,-fX-fY,fZ);
			pVertexNodeArr[6]=m_pTower->AppendNode();
			pVertexNodeArr[6]->SetLayer("SP7");
			pVertexNodeArr[6]->SetPosition(-fX,-fX-fY,fZ);
			pVertexNodeArr[7]=m_pTower->AppendNode();
			pVertexNodeArr[7]->SetLayer("SP8");
			pVertexNodeArr[7]->SetPosition(-fX-fY,-fX,fZ);
		}
		//显示节点
		...
		for(int j=0; j<nPoly; j++)
		{
			if(pPrevVertexNodeArr[j]==NULL)
			{
				pPrevVertexNodeArr[j] = pVertexNodeArr[j];
				continue;
			}
#if !defined(__TSA_)&&!defined(__TSA_FILE_)
			if(pDefine->m_iMainPartType==0)	//钢管
			{
				pLinePart=(CLDSLineTube*)m_pTower->AppendPart(CLS_LINETUBE);
				pLinePart->size_wide=140;
				pLinePart->size_thick=4;
			}
			else
				pLinePart=(CLDSLineAngle*)m_pTower->AppendPart(CLS_LINEANGLE);
#else
			pLinePart=(CLDSLinePart*)m_pTower->AppendPart(CLS_LINEPART);
			if(pDefine->m_iMainPartType==0)	//钢管
			{
...
#endif
			pLinePart->pStart=pPrevVertexNodeArr[j];
			pLinePart->pEnd=pVertexNodeArr[j];
			pLinePart->pStart->hFatherPart=pLinePart->handle;
			pLinePart->pEnd->hFatherPart=pLinePart->handle;
			pLinePart->CalPosition();
#ifdef __ANGLE_PART_INC_
			if(nPoly==4 && pLinePart->GetClassTypeId()==CLS_LINEANGLE)
			{
				char cDatumWing=0;
				if(pLinePart->pStart->layer(2)=='1'||pLinePart->pStart->layer(2)=='4')
					cDatumWing='X';
					...
					cDatumWing='Y';
				((CLDSLineAngle*)pLinePart)->m_cPosCtrlDatumLineType=cDatumWing;
				((CLDSLineAngle*)pLinePart)->m_cPosCtrlType=1;
				((CLDSLineAngle*)pLinePart)->desStartPos.spatialOperationStyle=19;	//旧式塔身控制点平推
				((CLDSLineAngle*)pLinePart)->desStartPos.cFaceOffsetDatumLine=cDatumWing;	//旧式塔身控制点平推
				if(i==pDefine->m_xOpenArr.GetSize()-1)
				{
					((CLDSLineAngle*)pLinePart)->desEndPos.spatialOperationStyle=19;	//旧式塔身控制点平推
					((CLDSLineAngle*)pLinePart)->desEndPos.cFaceOffsetDatumLine=cDatumWing;	//旧式塔身控制点平推
				}
				else
				{
					((CLDSLineAngle*)pLinePart)->desEndPos.spatialOperationStyle=4;
					((CLDSLineAngle*)pLinePart)->desEndPos.wing_x_offset.operStyle=1;
					((CLDSLineAngle*)pLinePart)->desEndPos.wing_x_offset.gStyle=4;
					((CLDSLineAngle*)pLinePart)->desEndPos.wing_y_offset.gStyle=4;
					((CLDSLineAngle*)pLinePart)->desEndPos.wing_x_offset.offsetDist=0;
					((CLDSLineAngle*)pLinePart)->desEndPos.wing_y_offset.offsetDist=0;
				}
			}
#endif
			//设置杆件图层名
			char layer[4]="SZ";
			layer[2]=pLinePart->pStart->layer(2);
			pLinePart->SetLayer(layer);
			linePartArr[j]=pLinePart;
			pPrevVertexNodeArr[j]=pVertexNodeArr[j];
		}
		//显示杆件
		if(i>0)
		{
			double piece_angle=(2*Pi)/nPoly;
			for(int j=0;j<nPoly;j++)
			{
#if !defined(__TSA_)&&!defined(__TSA_FILE_)
				if(pDefine->m_iMainPartType==0)
				{	//钢管
					//管塔主材基准面计算方式默认为共面参照点 wht 15-08-12
					linePartArr[j].LineTubePointer()->m_iNormCalStyle=2;
					linePartArr[j].LineTubePointer()->nearWorkPlaneNorm.Set(0,0,0);
				}
				else //if(pDefine->m_iMainPartType==1)
				{	//角钢
					linePartArr[j].LineAnglePointer()->des_norm_x.hViceJg=linePartArr[(j+nPoly-1)%nPoly]->handle;
					linePartArr[j].LineAnglePointer()->des_norm_x.near_norm.Set(sin(piece_angle*(j-1)),cos(piece_angle*(j-1)),0);
					linePartArr[j].LineAnglePointer()->des_norm_y.hViceJg=linePartArr[(j+1)%nPoly]->handle;
					linePartArr[j].LineAnglePointer()->des_norm_y.near_norm.Set(sin(piece_angle*j),cos(piece_angle*j),0);
					linePartArr[j].LineAnglePointer()->cal_x_wing_norm();
					linePartArr[j].LineAnglePointer()->cal_y_wing_norm();
				}
#endif
				NewLinePart(linePartArr[j]);	//必须在计算完肢法线后才能生成实体数据
				//填充对称关联信息
				for(int k=0; k<nPoly; k++)
				{
					if(j==k)	//跳过本身
