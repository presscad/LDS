...
public:
	...
	BOOL AttachCmdOptionPanel(CCmdLineMiniPanel* pPanel);
	void DetachCmdOptionPanel();
	BOOL AttachMiniCmdOptionPanel(CCmdLineMiniPanel* pMiniPanel,int cx);
	void DetachMiniCmdOptionPanel();
	int  ExecuteExternalCommand(CWnd* pCallWnd,UINT uiCmdType,DWORD dwParams);
	...
	static CHashSet<PROPLIST_ITEM*>hashCmdLineHelpItemById;
	//static void InitpropMenuHashtable();
	static PROPLIST_ITEM* AddCmdLineHelpItem(char* keyStr,PROPLIST_ITEM item);
