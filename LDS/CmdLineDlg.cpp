...
CHashStrList<PROPLIST_ITEM> CCmdLineDlg::propMenuHashtable;	//属性字符串哈希表
CHashSet<PROPLIST_ITEM*>CCmdLineDlg::hashCmdLineHelpItemById;
PROPLIST_ITEM* CCmdLineDlg::AddCmdLineHelpItem(char* keyStr,PROPLIST_ITEM item)
{
	return AddUniversalPropItem(keyStr,item,&propMenuHashtable,&hashCmdLineHelpItemById);
}
...
BOOL CCmdLineDlg::AttachCmdOptionPanel(CCmdLineMiniPanel* pPanel)
{
	if(m_pPanel!=pPanel&&pPanel&&pPanel->GetSafeHwnd()!=NULL)
		pPanel->DestroyWindow();
	m_pPanel=pPanel;
	pPanel->SetParentCmdLineDlg(this);

	RECT rcCmdHistory,rcPanel;
	CWnd *pCmdHistoryWnd = GetDlgItem(IDC_RCE_CMD_HISTORY);
	pCmdHistoryWnd->GetWindowRect(&rcCmdHistory);
	if(pPanel->GetSafeHwnd()==NULL)
		pPanel->Create(pPanel->DlgResourceID(),this);
	pPanel->GetWindowRect(&rcPanel);

	int height=rcPanel.bottom-rcPanel.top+1;
	int width=rcPanel.right-rcPanel.left+1;
	int xOffsetPanel=max(0,width-(rcCmdHistory.right-rcCmdHistory.left+1));

	ScreenToClient(&rcCmdHistory);
	rcPanel=rcCmdHistory;
	rcCmdHistory.right=rcCmdHistory.right-width+xOffsetPanel;
	int niHeightModification=0;
	rcPanel.left=rcCmdHistory.right+1;
	if(pPanel->GetFixedPanelHeight()>0)
	{	//需要设定为指定高度的选项面板
		int niBeforeHeight=rcCmdHistory.bottom-rcCmdHistory.top+1;
		pPanel->SetBeforePanelHeight(niBeforeHeight);
		int niFixedPanelHeight=pPanel->GetFixedPanelHeight();
		rcCmdHistory.top=rcPanel.top=rcCmdHistory.bottom-niFixedPanelHeight+1;
		niHeightModification=niFixedPanelHeight-niBeforeHeight;
	}
	else
		rcPanel.top=rcCmdHistory.bottom-height+1;
	CDialogPanel* pFrmPage=(CDialogPanel*)this->GetParent();
	CPaneDivider* pDivider=pFrmPage->GetDefaultPaneDivider();
	if(pDivider)
		pDivider->Move(CPoint(0,-niHeightModification));
	//else
	//	logerr.Log("error");	//不知道是何原因，有的计算机返回的pDivider可能为NULL wjh-2019.4.23
	pCmdHistoryWnd->MoveWindow(&rcCmdHistory);
	pPanel->MoveWindow(&rcPanel);
	pPanel->ShowWindow(SW_SHOW);
	return pPanel->GetSafeHwnd()!=NULL;
}
void CCmdLineDlg::DetachCmdOptionPanel()
{
	if(m_pPanel&&m_pPanel->GetSafeHwnd())
	{
		if(m_pPanel->GetFixedPanelHeight()>0)
		{	//需要设定为指定高度的选项面板
			RECT rcCmdHistory;
			CWnd *pCmdHistoryWnd = GetDlgItem(IDC_RCE_CMD_HISTORY);
			pCmdHistoryWnd->GetWindowRect(&rcCmdHistory);
			ScreenToClient(&rcCmdHistory);
			int niBeforePanelHeight=m_pPanel->GetBeforePanelHeight();
			int niCurrntPanelHeight=rcCmdHistory.bottom-rcCmdHistory.top+1;
			int niHeightModification=niBeforePanelHeight-niCurrntPanelHeight;
			rcCmdHistory.top=rcCmdHistory.bottom-niBeforePanelHeight+1;
			pCmdHistoryWnd->MoveWindow(&rcCmdHistory);
			CDialogPanel* pFrmPage=(CDialogPanel*)this->GetParent();
			CPaneDivider* pDivider=pFrmPage->GetDefaultPaneDivider();
			if(pDivider)
		...
		m_pPanel->SetParentCmdLineDlg(NULL);
...
int  CCmdLineDlg::ExecuteExternalCommand(CWnd* pCallWnd,UINT uiCmdType,DWORD dwParams)
{
	return 0;
}
...
void CCmdLineDlg::RelayoutWnd()
{
	...
	if(pCmdPromptWnd&&pCmdLineWnd&&pCmdHistoryWnd)
	{
		...
		if(m_pPanel!=NULL&&m_pPanel->GetSafeHwnd()!=NULL)
		{
			m_pPanel->MoveWindow(&rcCmdHistory);
			pCmdHistoryWnd->ShowWindow(SW_HIDE);
			//RECT rcPanel;
			//m_pPanel->GetWindowRect(&rcPanel);
			//this->ScreenToClient(&rcPanel);
			//rcCmdHistory.right=rcPanel.left-1;
		}
		else
		{
			pCmdHistoryWnd->ShowWindow(SW_SHOW);
			pCmdHistoryWnd->MoveWindow(&rcCmdHistory);
		}
...
BOOL CCmdLineDlg::GetStrFromCmdLine(CString &cmdStr,DWORD dwFlag/*=0*/,const char* keywords/*=NULL*/)//默认不响应鼠标左键抬起的消息
{
	...
	for(;;)
	{
		...
		if(PeekMessage(&message,NULL,0,0,PM_REMOVE))
		{
			...
			if(m_pPanel==NULL||!m_pPanel->IsDialogMessage(&message))
			{	//存在嵌入式对话框时需要使用IsDialogMessage进行消息处理，否则对话框内不支持Tab键 wht 19-01-24
...
void CCmdLineDlg::OnSelchangeCmdHistory(NMHDR* pNMHDR, LRESULT* pResult) 
{
	...
	CLDSView *pView=pMainFrm->GetLDSView();
	//得到当前选中行的索引号
	...
	g_pSolidDraw->SetEntSnapStatus(hFirstPart);
	g_pSolidDraw->SetEntSnapStatus(hSecPart);
	CLDSPart *pFirstPart=console.FromPartHandle(hFirstPart);
	//设置旋转中心
	if(pFirstPart)
	{
		if(pFirstPart->IsLinePart())
			g_pSolidSet->SetRotOrg(((CLDSLinePart*)pFirstPart)->Start());
		else
			g_pSolidSet->SetRotOrg(pFirstPart->ucs.origin);
		if(pView)
			pView->UpdatePropertyPage(pFirstPart);
	}
	//g_pSolidOper->ZoomWnd()
	*pResult = 0;
}
...
BOOL CCmdLineDlg::OnHelpInfo(HELPINFO* pHelpInfo) 
{
	UpdateData();
	CXhChar200 helpStr;
	if(m_sUserInputCmd.CompareNoCase("LegSchema")==0)
		helpStr.Copy("MultiLegSchema");
	else if(m_sUserInputCmd.CompareNoCase("CL")==0)
		helpStr.Copy("CopyLeg");
	else if(m_sUserInputCmd.CompareNoCase("DF")==0)
		helpStr.Copy("DesignFoot");
	else if(m_sUserInputCmd.CompareNoCase("CNFRC")==0)
		helpStr.Copy("CorrectNodeFatherRodCfgWord");
	else if(m_sUserInputCmd.CompareNoCase("DH")==0)
		helpStr.Copy("Hoof");
	else if(m_sUserInputCmd.CompareNoCase("N")==0)
		helpStr.Copy("NewTower");
	else if(m_sUserInputCmd.CompareNoCase("NS")==0)
		helpStr.Copy("NewSubstation");
	else if(m_sUserInputCmd.CompareNoCase("H")==0)
		helpStr.Copy("Hide");
	else if(m_sUserInputCmd.CompareNoCase("Re")==0)
		helpStr.Copy("Refresh");
	else if(m_sUserInputCmd.CompareNoCase("L")==0)
		helpStr.Copy("Link");
	else if(m_sUserInputCmd.CompareNoCase("F")==0)
		helpStr.Copy("Find");
	else if(m_sUserInputCmd.CompareNoCase("B")==0)
		helpStr.Copy("Break");
	else if(m_sUserInputCmd.CompareNoCase("GY")==0)
		helpStr.Copy("GongYongBan");
	else if(m_sUserInputCmd.CompareNoCase("MN")==0)
		helpStr.Copy("MergeNodes");
	else
		helpStr.Copy(m_sUserInputCmd);
	PROPLIST_ITEM* pHelpItem=propMenuHashtable.GetValue(helpStr);
	if(pHelpItem==NULL)
		return FALSE;
	else
	{
		helpStr.Copy(theApp.m_pszHelpFilePath);
		if(pHelpItem->Url.GetLength()>0)
			helpStr.Append(CXhChar100("::/%s",(char*)pHelpItem->Url));
