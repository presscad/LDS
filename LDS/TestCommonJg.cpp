...
double CLDSLineAngle::Test_GetStartExtraOdd(int wing_x0_y1_ber2)
{
	...
	else if(wing_x0_y1_ber2==0&&extraStartOdd.wing_x.style==1)
	{
		...
		extraStartOdd.wing_x.extraDatumPoint.UpdatePos(m_pTower);
		f3dPoint start=Start();//extraStartOdd.wing_x.extraStartPoint.pos;
		f3dPoint end=extraStartOdd.wing_x.extraDatumPoint.pos;
	...
	else if(wing_x0_y1_ber2==1&&extraStartOdd.wing_y.style==1)
	{
		...
		extraStartOdd.wing_y.extraDatumPoint.UpdatePos(m_pTower);
		f3dPoint start=Start();//extraStartOdd.wing_y.extraStartPoint.pos;
		f3dPoint end=extraStartOdd.wing_y.extraDatumPoint.pos;
		f3dPoint direct=End()-Start();
		normalize(direct);
		f3dPoint odd_vec=end-start;
	...
	else if(wing_x0_y1_ber2==2&&extraStartOdd.ber.style==1)
	{
		...
		extraStartOdd.ber.extraDatumPoint.UpdatePos(m_pTower);
		f3dPoint start=Start();//extraStartOdd.ber.extraStartPoint.pos;
		f3dPoint end=extraStartOdd.ber.extraDatumPoint.pos;
		f3dPoint direct=End()-Start();
		normalize(direct);
		f3dPoint odd_vec=end-start;
	...
	else
	{
		...
		else	//两步偏移
		{
			...
			{
				CLDSLineAngle *pHuoQuJg=(CLDSLineAngle*)SuperiorTower()->Parts.FromHandle(pOldDesStartPos->huoqu_jg_h,CLS_LINEANGLE,CLS_GROUPLINEANGLE);
				if(pHuoQuJg==NULL)
					pHuoQuJg=(CLDSLineAngle*)SuperiorTower()->Parts.FromHandle(pStart->hFatherPart,CLS_LINEANGLE,CLS_GROUPLINEANGLE);
				if(pHuoQuJg==NULL)
					return 0;
...
double CLDSLineAngle::Test_GetEndExtraOdd(int wing_x0_y1_ber2)
{
	...
	else if(wing_x0_y1_ber2==0&&extraEndOdd.wing_x.style==1)
	{
		...
		extraEndOdd.wing_x.extraDatumPoint.UpdatePos(m_pTower);
		f3dPoint start=End();//extraEndOdd.wing_x.extraStartPoint.pos;
		f3dPoint end=extraEndOdd.wing_x.extraDatumPoint.pos;
	...
	else if(wing_x0_y1_ber2==1&&extraEndOdd.wing_y.style==1)
	{
		...
		extraEndOdd.wing_y.extraDatumPoint.UpdatePos(m_pTower);
		f3dPoint start=End();//extraEndOdd.wing_y.extraStartPoint.pos;
		f3dPoint end=extraEndOdd.wing_y.extraDatumPoint.pos;
	...
	else if(wing_x0_y1_ber2==2&&extraEndOdd.ber.style==1)
	{
		...
		extraEndOdd.ber.extraDatumPoint.UpdatePos(m_pTower);
		f3dPoint start=End();//extraEndOdd.ber.extraStartPoint.pos;
		f3dPoint end=extraEndOdd.ber.extraDatumPoint.pos;
		f3dPoint direct=Start()-End();
		normalize(direct);
		f3dPoint odd_vec=end-start;
	...
	else
	{
		...
		else	//两步偏移
		{
			...
			{
				CSmartPtr<CLDSLineAngle> pHuoQuJg=(CLDSLineAngle*)SuperiorTower()->Parts.FromHandle(pOldDesEndPos->huoqu_jg_h,CLS_LINEANGLE,CLS_GROUPLINEANGLE);
				if(pHuoQuJg.IsNULL())
					pHuoQuJg=(CLDSLineAngle*)SuperiorTower()->Parts.FromHandle(pEnd->hFatherPart,CLS_LINEANGLE,CLS_GROUPLINEANGLE);
				if(pHuoQuJg.IsNULL())
					return 0;
				f3dPoint work_norm=((pHuoQuJg->End()-pHuoQuJg->Start())^(End()-Start()));
				normalize(work_norm);
...
f3dPoint CLDSLineAngle::Test_GetStartDatumPos()
{
	...
	if(pOldDesStartPos->jgber_cal_style==0)
	{
		...
		if(pOldDesStartPos->datum_jg_h>0x20)
			pDatumRod=(CLDSLinePart*)SuperiorTower()->Parts.FromHandle(pOldDesStartPos->datum_jg_h,CLS_LINEPART);
		if(pDatumRod.IsNULL()&&pStart!=NULL)
			pDatumRod=(CLDSLinePart*)SuperiorTower()->Parts.FromHandle(pStart->hFatherPart,CLS_LINEPART);
		...
		if(pOldDesStartPos->datum_pos_style==0&&pStart!=NULL)		//节点即基点
			...
			return pOldDesStartPos->datum_point.pos;
		if(pDatumRod.IsNULL())
...
f3dPoint CLDSLineAngle::Test_GetEndDatumPos()
{
	...
	if(pOldDesEndPos->jgber_cal_style==0)
	{
		...
		if(pOldDesEndPos->datum_jg_h>0x20)
			pDatumRod=(CLDSLinePart*)SuperiorTower()->Parts.FromHandle(pOldDesEndPos->datum_jg_h,CLS_LINEPART);
		if(pDatumRod.IsNULL()&&pEnd!=NULL)
			pDatumRod=(CLDSLinePart*)SuperiorTower()->Parts.FromHandle(pEnd->hFatherPart,CLS_LINEPART);
		f3dPoint len_offset_vec;
		if(pDatumRod.IsHasPtr())
		{
			len_offset_vec=pDatumRod->End()-pDatumRod->Start();
		...
		if(pOldDesEndPos->datum_pos_style==0&&pEnd!=NULL)
			...
			return pOldDesEndPos->datum_point.pos;
...
f3dPoint CLDSLineAngle::Test_GetStartDatumPosEx()
{
	//try{
		...
		{	//搭接角钢
			...
			CSmartPtr<CLDSLineAngle>pDatumJg;
			if(pOldDesStartPos->datum_jg_h>0x20)
				pDatumJg=(CLDSLineAngle*)SuperiorTower()->Parts.FromHandle(pOldDesStartPos->datum_jg_h,CLS_LINEANGLE,CLS_GROUPLINEANGLE);
			if(pDatumJg.IsNULL())
				pDatumJg=(CLDSLineAngle*)SuperiorTower()->Parts.FromHandle(pStart->hFatherPart,CLS_LINEANGLE,CLS_GROUPLINEANGLE);
			if(pDatumJg.IsNULL())
...
f3dPoint CLDSLineAngle::Test_GetEndDatumPosEx()
{
	//try{
		...
		{	//搭接角钢
			...
			CSmartPtr<CLDSLineAngle>pDatumJg;
			if(pOldDesEndPos->datum_jg_h>0x20)
				pDatumJg=(CLDSLineAngle*)SuperiorTower()->Parts.FromHandle(pOldDesEndPos->datum_jg_h,CLS_LINEANGLE,CLS_GROUPLINEANGLE);
			if(pDatumJg.IsNULL())
				pDatumJg=(CLDSLineAngle*)SuperiorTower()->Parts.FromHandle(pEnd->hFatherPart,CLS_LINEANGLE,CLS_GROUPLINEANGLE);
			if(pDatumJg.IsNULL())
...
{
	SuperiorTower()->Parts.push_stack();
	for(CLDSLineAngle *pLineAngle=(CLDSLineAngle*)SuperiorTower()->Parts.GetFirst(CLS_LINEANGLE);pLineAngle;
		pLineAngle=(CLDSLineAngle*)SuperiorTower()->Parts.GetNext(CLS_LINEANGLE))
	{
...
void CLDSLineAngle::Test_UpdateConnectInfo(CLDSNode *pNode)
{
	...
	else
	{
		...
		pDatumLinePart=(CLDSLinePart*)SuperiorTower()->Parts.FromHandle(hFatherPart);
