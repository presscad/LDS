// InsertStdTemplateOperation.cpp: implementation of the CInsertStdTemplateOperation class.
...
CLDSLinePart* CInsertStdTemplateOperation::InsertNewPole(CParaRod *pParaRod)
{
	...
	else
	{
		...
		if(pPole->layer(1)=='B')
			pPole->layer(1)='F';
		if(pPole->IsAuxPole())
			pPole->connectStart.wnConnBoltN=pPole->connectEnd.wnConnBoltN=1;	//辅材默认一颗螺栓，以便于自动拆分节点和批量端螺栓设计 wjh-2017.12.17
	...
	CParaTemplate* pParaTemplate,CLDSNode* keyNodeArr[4],int keyNodeMir,int templateMir,const double* axis_x_coords/*=NULL*/)
{
	...
	for(CParaDatumPlane* pParaPlane=pParaTemplate->listParaPlanes.GetFirst();pParaPlane;pParaPlane=pParaTemplate->listParaPlanes.GetNext())
	{
		...
		pMapPlane->pMappingPlane=Ta.Plane.append();
		StrCpy(pMapPlane->pMappingPlane->description,pParaPlane->description);
		CLDSPoint* pPoint1=Ta.Point.append();
		CLDSPoint* pPoint2=Ta.Point.append();
		CLDSPoint* pPoint3=Ta.Point.append();
													...
													CFGWORD* pBodyWord/*=NULL*/,CFGWORD* pLegWord/*=NULL*/)
{
	...
	CLDSNode *pLdsNode;
	long hInitHandle=Ta.handle;
#ifdef __ANGLE_PART_INC_//!defined(__TSA_)&&!defined(__TSA_FILE_)
	CLDSLineAngle *pAngle;
#endif
	CLDSLinePart *pLinePart;
	CFacePanelNode _arrFaceNodePool[200];
	CFacePanelPole _arrFacePolePool[200];
	ARRAY_LIST<CFacePanelNode>arrFaceNodeTemp(_arrFaceNodePool,200,50);
	ARRAY_LIST<CFacePanelPole>arrFacePoleTemp(_arrFacePolePool,200,50);
	...
	m_arrTemplateLdsLinePart.Empty();//原始生成的标准段或标准面内的杆件集合
	m_arrMirSumLdsLinePart.Empty();	//含原始生成及对称生成在内(不含镜像生成)的标准段或标准面内的杆件集合
	CLDSNodePtr _arrTmplLdsNodesPool[500],_arrMirSumLdsNodesPool[1000];
	CLDSLinePartPtr _arrTmplLdsRodsPool[500],_arrMirSumLdsRodsPool[1000];
	m_arrTemplateLdsNode.Attach(_arrTmplLdsNodesPool, 0, 500);
	m_arrMirSumLdsNode.Attach(_arrMirSumLdsNodesPool, 0, 1000);
	m_arrTemplateLdsLinePart.Attach(_arrTmplLdsRodsPool, 0, 500);
	m_arrMirSumLdsLinePart.Attach(_arrMirSumLdsRodsPool, 0, 1000);
	...
	{
		pFacePole=arrFacePoleTemp.append(*pFacePole);
		if(pPanelInfo->blFlipHorizon)	//需左右面翻转角钢肢朝向 wjh-2019.4.24
			pFacePole->wxLayout.ciLayoutStyle=5-pFacePole->wxLayout.ciLayoutStyle;
	}
	int cMirTransX1Y2=0;
	...
	for(i=0;i<repeatNum;i++)
	{
		...
		if(pActiveView&&near_norm.IsZero())
			near_norm=pActiveView->ucs.axis_z;
		...
		{
			/*if(m_pFacePanel->m_iFaceType==1&&j<2)
				continue;	//塔身材已有不需要生成
			else if(m_pFacePanel->m_iFaceType==3&&!arrFacePoleTemp[j].m_bInternodeRepeat)
				continue;	//多节间特殊模板面中的轮廓面,已有不在此生成*/
			...
			else
			{
				...
				if(pLinePart->IsAuxPole())
					pLinePart->connectStart.wnConnBoltN=pLinePart->connectEnd.wnConnBoltN=1;	//辅材默认一颗螺栓，以便于自动拆分节点和批量端螺栓设计 wjh-2017.12.17
				...
				pLinePart->des_wing_norm.wLayoutStyle=arrFacePoleTemp[j].wxLayout.ciLayoutStyle;
				//避免直接对ANGLE_LAYOUT赋整数以免因为结构体内部属性顺序调整导致赋值错误　wjh-2019.4.23
				ANGLE_LAYOUT wxAngleLayStyle((BYTE)pLinePart->des_wing_norm.wLayoutStyle,0);
				...
				if(j>0)
				{
					...
					//g_pSolidDraw->SetEntSnapStatus执行起来太慢也没必要 wjh-2019.8.1
		...
		for(j=0;j<m_arrTemplateLdsNode.GetSize();j++)
		{
			if(m_arrTemplateLdsNode[j]->m_cPosCalType==4)	//交叉点
			{
				...
				for(int k=0;k<m_arrTemplateLdsLinePart.GetSize();k++)
				{
					...
					if(line.PtInLine(m_arrTemplateLdsNode[j]->Position(false))<=0)
						continue;
					//判断arrFaceNodeTemp[j].attach_node_index[0,1,2,3]是否大于0，否则插入模板异常 wht 18-11-09
					else if(arrFaceNodeTemp[j].attach_node_index[0]>=0&&line.PtInLine(m_arrTemplateLdsNode[arrFaceNodeTemp[j].attach_node_index[0]]->Position(false))>0
						 && arrFaceNodeTemp[j].attach_node_index[1]>=0&&line.PtInLine(m_arrTemplateLdsNode[arrFaceNodeTemp[j].attach_node_index[1]]->Position(false))>0)
						m_arrTemplateLdsNode[j]->arrRelationPole[0]=m_arrTemplateLdsLinePart[k]->handle;
					else if(arrFaceNodeTemp[j].attach_node_index[2]>=0&&line.PtInLine(m_arrTemplateLdsNode[arrFaceNodeTemp[j].attach_node_index[2]]->Position(false))>0
						 && arrFaceNodeTemp[j].attach_node_index[3]>=0&&line.PtInLine(m_arrTemplateLdsNode[arrFaceNodeTemp[j].attach_node_index[3]]->Position(false))>0)
			...
			{	//无父杆件信息
				...
				if(pViceFatherPole!=NULL&&m_arrTemplateLdsNode[j]->hFatherPart<0x20)	//未找到父杆件，则以第一根准父杆件为父杆件
				{
		...
		for(j=0;j<m_arrTemplateLdsNode.GetSize();j++)
		{
			if(m_arrTemplateLdsNode[j]->handle>=hInitHandle&&m_arrTemplateLdsNode[j]->IsLegObj()&&pActiveModule)
			{
				...
				if(m_arrTemplateLdsNode[j]->Layer(2)>'1'&&m_arrTemplateLdsNode[j]->Layer(2)<='4'&&pActiveModule)
				{	
	...
	char ciLegQuad=0;
	if(pFacePanel->m_iFaceType==6)
	{
		if(m_arrTemplateLdsNodeArr[0]->xOriginalPos.x>0&&m_arrTemplateLdsNodeArr[0]->xOriginalPos.y>0)
			ciLegQuad=1;
		else if(m_arrTemplateLdsNodeArr[0]->xOriginalPos.x<0&&m_arrTemplateLdsNodeArr[0]->xOriginalPos.y>0)
			ciLegQuad=2;
		else if(m_arrTemplateLdsNodeArr[0]->xOriginalPos.x<0&&m_arrTemplateLdsNodeArr[0]->xOriginalPos.y<0)
			ciLegQuad=4;
		else if(m_arrTemplateLdsNodeArr[0]->xOriginalPos.x>0&&m_arrTemplateLdsNodeArr[0]->xOriginalPos.y<0)
...
bool CInsertStdTemplateOperation::MirTemplate(char ciQuad/*=0*/)
{
	if(m_iTemplateMir==0)
		return false;	//无须对称
	//根据构件的类型进行排序
	...
	else if(m_iTemplateMir==6)	//角对称
	{
		mirmsg.axis_flag=0x10;
		mirmsg.array_num=1;
		mirmsg.rotate_angle=0;
