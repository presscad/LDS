...
bool CIntelliCore::AddHeadType_ZB2A(STDTEMPL *pStdTempl)
{
	...
	pRod->ciAngleLayout=2;
	pRod->widRefRodOfWorkWing=9;
	pRod=pStdTempl->hashParaRods.Add(30);	//右侧地线支架T型组合水平挂线角钢
	pRod->widStartNode= 7;
	pRod->widEndNode  =-7;
	...
	pRod=pStdTempl->hashParaRods.Add(31);	//左侧地线支架T型组合水平挂线角钢
	...
	pRod->ciSectStyle=2;
	pRod=pStdTempl->hashParaRods.Add(32);	//右侧导线支架T型组合水平挂线角钢
	pRod->widStartNode= 4;
	pRod->widEndNode  =-4;
	pRod->ciNearNormAxis=5;
	pRod->ciAngleLayout=3;
	...
	pRod->ciSectStyle=2;
	pRod->ciMasterIndex=1;
	pRod=pStdTempl->hashParaRods.Add(33);	//左侧导线支架T型组合水平挂线角钢
	...
	pQuadSect->arrRods[1]=-9;
	pQuadSect=pStdTempl->listQuadSects.AttachObject();
	pQuadSect->arrRods[0]=27;
	pQuadSect->arrRods[1]=-9;
	//约束条件
	//pStdTempl->condition.CompileExprStr("Abs(Node#3.x-Node#9.x)<100");
	return true;
}
#endif
