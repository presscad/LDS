...
////////////////////////////////////////////////////////////////////////////////
CImageFileJpeg::CxExifInfo::CxExifInfo(EXIFINFO* info)
{
	if (info) {
		m_exifinfo = info;
		freeinfo = false;
...
////////////////////////////////////////////////////////////////////////////////
CImageFileJpeg::CxExifInfo::~CxExifInfo()
{
	for(int i=0;i<MAX_SECTIONS;i++) if(Sections[i].Data) free(Sections[i].Data);
	if (freeinfo) delete m_exifinfo;
}
////////////////////////////////////////////////////////////////////////////////
bool CImageFileJpeg::CxExifInfo::DecodeExif(FILE *fp, int nReadMode)
{
    int a;
    int HaveCom = FALSE;
    ...
    for(;;){
        ...
        got = fread(Data+2, 1, itemlen-2,fp); // Read the whole section.
        if (got != itemlen-2){
...
--------------------------------------------------------------------------*/
bool CImageFileJpeg::CxExifInfo::process_EXIF(unsigned char * CharBuf, unsigned int length)
{
...
//--------------------------------------------------------------------------
int CImageFileJpeg::CxExifInfo::Get16m(void * Short)
{
...
--------------------------------------------------------------------------*/
int CImageFileJpeg::CxExifInfo::Get16u(void * Short)
{
    if (MotorolaOrder){
        return (((unsigned char *)Short)[0] << 8) | ((unsigned char *)Short)[1];
    }else{
...
--------------------------------------------------------------------------*/
long CImageFileJpeg::CxExifInfo::Get32s(void * Long)
{
    if (MotorolaOrder){
...
--------------------------------------------------------------------------*/
unsigned long CImageFileJpeg::CxExifInfo::Get32u(void * Long)
{
...
--------------------------------------------------------------------------*/
bool CImageFileJpeg::CxExifInfo::ProcessExifDir(unsigned char * DirStart, unsigned char * OffsetBase, unsigned ExifLength,
...
--------------------------------------------------------------------------*/
double CImageFileJpeg::CxExifInfo::ConvertAnyFormat(void * ValuePtr, int Format)
{
    double Value;
    Value = 0;
...
////////////////////////////////////////////////////////////////////////////////
void CImageFileJpeg::CxExifInfo::process_COM (const BYTE * Data, int length)
{
    int ch;
...
////////////////////////////////////////////////////////////////////////////////
void CImageFileJpeg::CxExifInfo::process_SOFn (const BYTE * Data, int marker)
{
...
////////////////////////////////////////////////////////////////////////////////
/**
 * this will work only on a CImageFileJpeg object, if the image originally has valid EXIF data
 \verbatim
	CImageFileJpeg jpg;
	CxIOFile in,out;
 ...
 \endverbatim
*/
bool CImageFileJpeg::CxExifInfo::EncodeExif(FILE *fp)
{
    int a;
    ...
    if (Sections[0].Type != M_EXIF && Sections[0].Type != M_JFIF){
        ...
        fwrite(JfifHead, 18, 1,fp);
    }

    // Write all the misc sections
    for (a=0;a<SectionsRead-1;a++){
        fputc(0xff,fp);
        fputc((unsigned char)(Sections[a].Type),fp);
        fwrite(Sections[a].Data, Sections[a].Size, 1,fp);
    }
...
////////////////////////////////////////////////////////////////////////////////
void CImageFileJpeg::CxExifInfo::DiscardAllButExif()
{
...
////////////////////////////////////////////////////////////////////////////////
void* CImageFileJpeg::CxExifInfo::FindSection(int SectionType)
{
    int a;
    for (a=0;a<SectionsRead-1;a++){
