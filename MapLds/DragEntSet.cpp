...
bool CDragEntSet::BeginBlockRecord(const char* blkname/*=NULL*/)
{
	...
	CXhChar200 sBlkName;
	AcDbObjectId blkidtemp;
	if(blkname!=NULL)
		blkidtemp = SearchBlock(blkname);
	if(blkname==NULL||!blkidtemp.isNull())
...
#ifdef _ARX_2007
	status=m_pRecordingBlockTableRecord->setName(_bstr_t(sBlkName));
#else
	status=m_pRecordingBlockTableRecord->setName(sBlkName);
#endif
	if(status!=Acad::eOk)
		AfxMessageBox("error");
