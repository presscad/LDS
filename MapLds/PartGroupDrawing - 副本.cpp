...
static void trans_pos(f3dPoint &pt,f3dPoint origin,double coef=1.0)
{
	pt+=origin;
	pt.x*=coef;
}
static void trans_pos(f3dLine &line,f3dPoint origin,double coef=1.0)
{
...
static void trans_pos(f3dArcLine &line,f3dPoint origin,double coef=1.0)
{
...
//CDrawingEntsContext
/////////////////////////////////////////////////////////////////////////////////
CSectionViewDrawing* CDrawingEntsContext::ValidSectDrawing(){
	...
	if(cStoreType==XERO_COMMAND)
		...
		pValidSectDrawing=context.pSectDrawing;
...
void CDrawingEntsContext::InitContext(CSectionViewDrawing* pXeroComponent,STORE_MODE cStoreMode/*=STORE_ACAD*/)
{
	//m_pRootPart=pRootPart;
...
double CDrawingEntsContext::ZoomToModelTextSize(double fSysFontSize)
{
	if(cStoreType==XERO_COMPONENT&&context.pSectDrawing!=NULL)
		return context.pSectDrawing->ZoomToModelTextSize(fSysFontSize);
...
void CDrawingEntsContext::
	AppendAngularDim(f3dPoint vertex,f3dPoint start_vec,f3dPoint end_vec,double fDimR/*=0*/,char *dimText/*=NULL*/)
{
	CSectionViewDrawing* pSectDrawing=ValidSectDrawing();
	if(m_cStoreMode==STORE_ACAD && pSectDrawing==NULL)
		return ;//NULL;
...
void CDrawingEntsContext::AppendSizeTextDim(CSizeTextDim& sizeTextDim)
{
	...
	if(m_cStoreMode==STORE_ACAD && pSectDrawing==NULL)
		return;
	if(m_cStoreMode==STORE_ACAD)
		return pSectDrawing->AppendSizeTextDim(sizeTextDim);
	else if(m_cStoreMode==STORE_ACADINSTANT)
	{
		//UNDONE#完成直接添加到ACAD块表记录里的代码
	}
	...
	{
		IDbRotatedDimension* pRotateDim=(IDbRotatedDimension*)AppendDbEntity(IDbEntity::DbRadialDimension);
...
{
	if(width<=0)
		width=sys.general_set.fWeldLineSymbolSpace;
	if(height<=0)
		height=sys.general_set.fWeldLineSymbolSpace;
...
double CPartGroupDrawing::AnalyzeLenModulate()
{
	...
	double majorsect_drawlength=majorsect_length/sys.general_set.WeldMapScale.fRealToDraw;
	...
	xDrawSet.fPartGroupDistZoomCoef*=sys.general_set.WeldMapScale.fRealToDraw;
	double zoomlen= 0;
	for(i=0;i<sectionList.GetNodeNum();i++)
	{
		if(i==0)
...
bool CPartGroupDrawing::InitPartGroupDrawing(CLDSPartGroup *pPartGroup,double maxDrawingLength,double maxDrawingHeight)
{
	...
	if(m_pRootPart->IsLinePart())
	{
		...
		if(xDrawSet.bModulatePartGroupLen&&pRootLinePart->GetLength()>4000)
			AnalyzeLenModulate();
	}
	else
		cs = m_pRootPart->ucs;
...
bool CPartGroupDrawing::CreatePartGroupDrawing()
{
	...
	for(pSectionView=m_pPartGroup->SonViews.GetFirst();pSectionView;pSectionView=m_pPartGroup->SonViews.GetNext())
		CreateSectionViewDrawing(pSectionView);
...
CSectionViewDrawing* CPartGroupDrawing::CreateSectionViewDrawing(CSectionView *pSectionView)
{
...
void CDimAngularManager::CreateAngularItems(const double* normal,const double* cStartEdgeVec,const double* vTubeLenVec,RADIAL_ITEM* items,int count)
{
	...
	for(i=0;i<count;i++)
	{
		if(items[i].radAngle<0.0177||(2*3.14-items[i].radAngle)<0.0177)	//0.0177≈1°
		{
			RADIAL_ITEM start_item;
			start_item.rayVec=m_vStartEdgeAxisVec;
			start_item.rayOrg=items[i].rayOrg;
			start_item.radAngle=items[i].radAngle;
			itemArr.append(start_item);
		}
		else if(items[i].radAngle<(2*3.14-0.01744))		//与起始边重合不做处理
	...
	for(i=0;i<nSize;i++)
	{
		...
		if(half_angle<1.57)
			fDimR = min((m_fDimAngleTextSize*0.7*4)/tan(half_angle),100);
		if(fMaxDimR<fDimR)
			fMaxDimR=fDimR;
	}
	for(i=0;i<nSize;i++)
	{
		...
		for(int next=i;next!=i&&next<=nSize&&pNextRadialItem->radAngle-pRadialItem->radAngle<0.1744;next++)
		{	//>10°
			pNextRadialItem=&itemArr[next%nSize];
			next--;
		}
...
BOOL CSectionViewDrawing::GetDCS(long hPart,CDrawCoordTrans &dcs)
{
	...
	else if(m_pSectionView->IsDualTubeJointView())//m_idViewType==CSectionView::CLS_DUALTUBE_JOINT_VIEW||m_pSectionView->m_idViewType==CSectionView::CLS_HORIZ_BEAM_VIEW)
	{
		...
		if(pAssist!=NULL)
		{
			...
			{
				CSectionView::ROTATION rotation;
				GECS rotcs;
...
BOOL CPlateDrawing::CreatePlateDrawing(CLDSPlate *pPlate,CDrawCoordTrans& dcs,IDbEntsContext* pContext,AcDbObjectId *pOutEntId/*=NULL*/)
{
	style.cbDrawPlanes=0x07;	//0x01:绘制基面;0x02:绘制1号火曲面;0x04:绘制2号火曲面
	if(pPlate->face_N>1 && (pPlate->designInfo.iFaceType==6||pPlate->designInfo.iFaceType==7))
	...
	UCS_STRU cs=dcs.cs;
	//初始化带边框文本
	data_point.InitTextFrame(pContext->ZoomToModelTextSize(sys.fonts.weld.fPartNoTextSize),pContext->ZoomToModelTextSize(sys.dim_map.PartNo.fPartNoCirD),-1,pContext->ZoomToModelTextSize(sys.dim_map.PartNo.fPartNoMargin));
	...
	if(pContext->IsSectionDrawing()&&pContext->WeldRootPart()==pPlate&&h0v1==0)
		...
		{	//水平绘制
			...
			else if(pStartVertex->type==3)	//椭圆弧
			{
				...
				else
				{
					...
					if(fabs(pPlate->ucs.axis_z*dcs.cs.axis_z)>EPS_COS)
	...
	else if(h0v1==1)
	{
		...
		int minDrawModelThick=pContext->TransToModelLength(sys.part_group_map.fMinDrawThick);
	...
	{	//水平绘制
		...
		for(PROFILE_VERTEX *pVertex=pPlate->vertex_list.GetFirst();pVertex;pVertex=pPlate->vertex_list.GetNext())
		{
			...
			{	//焊接父构件为钢管且存在对活点信息才需要标注对活点
				...
				locate_point.dimPos=midLine.startPt+(sign*pContext->TransToModelLength(sys.dim_map.fDimTextSize*0.5))*line_vec;//startLine.startPt;
	...
	if(h0v1==0)
	{
		for(CLsRef *pLsRef=pPlate->GetFirstLsRef();pLsRef;pLsRef=pPlate->GetNextLsRef())
		{
			...
			if(style.bSpreadBendPlane)
			{
				pPlate->SpreadLsPos(pLsRef,ls_cir.centre);
				coord_trans(ls_cir.centre,pPlate->ucs,TRUE,TRUE);
			}
			else
				pPlate->GetBoltIntersPos(pLsRef->GetLsPtr(),ls_cir.centre);
			ls_cir.centre=dcs.TransToDraw(ls_cir.centre);//coord_trans(ls_cir.centre,cs,FALSE);ls_cir.centre.z=0;	//z坐标归零
			ls_cir.radius=(*pLsRef)->get_d()*0.5;
			ls_cir.feature=ftoi((*pLsRef)->hole_d_increment*10);
			pContext->AppendCircle(ls_cir);
			if(pEntsManager)
				pEntsManager->UpdateVerticalPlateLocateBoltPos(GEPOINT(ls_cir.centre.x,ls_cir.centre.y,0));
		}
		if(pDatumTube&&style.bDimRayRodAngle)	//分析钢板射线杆件
			AnalizePoleRay(pPlate,dcs,pContext);
		if(style.bDimBoltSpace)
		{
			...
			CSizeTextDim space_dim;
			for(ALIGNDIM_STRU* pAlignDim=dimLs.EnumFirstAlignDim();pAlignDim;pAlignDim=dimLs.EnumNextAlignDim())
			{
				space_dim.dimStartPos=pAlignDim->startPos;
				space_dim.dimEndPos=pAlignDim->endPos;
				...
				space_dim.dimStartPos.z=space_dim.dimEndPos.z=0;
				space_dim.dist=pAlignDim->dist;
				space_dim.bAllowRelayout=FALSE;
				space_dim.dimPos=0.5*(space_dim.dimStartPos+space_dim.dimEndPos);
				space_dim.angle = atan((space_dim.dimEndPos.y-space_dim.dimStartPos.y)/(space_dim.dimEndPos.x-space_dim.dimStartPos.x));
				space_dim.iOrder = 0;	//装配检测线尺寸
				space_dim.nHits=pAlignDim->nLsSpaceNum;
				pContext->AppendSizeTextDim(space_dim);
				//spaceDimList.append(space_dim);
			}
			if(pEntsManager)
			{
				IDimPlateBolts::BOLTRAY *pBoltRay=NULL;
				for(pBoltRay=dimLs.EnumFirstBoltRay();pBoltRay;pBoltRay=dimLs.EnumNextBoltRay())
				{
					IDimPlateBolts::BOLT* pFirstBolt =pBoltRay->ls_arr.EnumObjectFirst();
					IDimPlateBolts::BOLT* pSecondBolt=pBoltRay->ls_arr.EnumObjectNext();
					if(pFirstBolt==NULL||pSecondBolt==NULL)
						continue;
					GELINE line(pFirstBolt->location,pSecondBolt->location);
					coord_trans(line.start,pPlate->ucs,TRUE);
					coord_trans(line.end,pPlate->ucs,TRUE);
					line.start=dcs.TransToDraw(line.start);
					line.end=dcs.TransToDraw(line.end);
					line.start.z=line.end.z=0;
...
BOOL CSectionViewDrawing::CreatePlateDrawing(CLDSPlate *pPlate,CDrawCoordTrans& dcs,BOOL bCoplanar/*=TRUE*/,
											 ...
											 BYTE geomcenter0_left1_right2/*=0*/)
{
	...
	if(m_pRootPart && m_pRootPart->GetClassTypeId()==CLS_SPHERE)	//球节点阻焊件中的单面板不需标注板的射线方向或角度
		drawer.style.bDimRayRodAngle=false;
	BOOL rslt=drawer.CreatePlateDrawing(pPlate,dcs,&m_xEntManager);
...
BOOL CSectionViewDrawing::CreateParamPlateDrawing(CLDSParamPlate *pPlate,CDrawCoordTrans& dcs,BOOL bCoplanar/*=TRUE*/,BOOL bDrawWeldLineByWeldProp/*=TRUE*/)
{
	...
	if(pPlate->m_iParamType==TYPE_UEND||pPlate->m_iParamType==TYPE_ROLLEND)
	{
		...
		else if(h0v1==1)	//绘制侧面
		{
			...
			//数据信息扩展点位置
			data_point.dataPos=point+1.5*M;
	...
	{	//组焊件中 暂不显示肘板上的孔 wht 09-12-20
		...
		for(CLsRef *pLsRef=pPlate->GetFirstLsRef();pLsRef;pLsRef=pPlate->GetNextLsRef())
		{
			...
			if(fabs(pLsRef->GetLsPtr()->get_norm()*pPlate->ucs.axis_z)>EPS_COS2)
			{
				...
				Int3dlf(ls_cir.centre,(*pLsRef)->ucs.origin,(*pLsRef)->ucs.axis_z,pPlate->ucs.origin,pPlate->ucs.axis_z);
...
BOOL CSectionViewDrawing::CreateLineTubeDrawing(CLDSLineTube *pLineTube,CDrawCoordTrans dcs,BOOL bDrawWeldLine/*=TRUE*/)
{
	...
	BOOL rslt=drawer.CreateTubeDrawing(pLineTube,dcs,&manager);
...
BOOL CSectionViewDrawing::CreateCutTubeDrawing(CLDSLineTube *pLineTube,BOOL bCoplanar/*=TRUE*/)
{
	...
	int str_len=strlen(pLineTube->GetPartNo());
...
BOOL CSectionViewDrawing::CreateLineAngleDrawing(CLDSLineAngle *pLineAngle,CDrawCoordTrans dcs)
{
	...
	{	//角钢脚钉视图添加数据信息扩展点
		...
		int str_len=strlen(pLineAngle->GetPartNo());
...
BOOL CSectionViewDrawing::CreateLineSlotDrawing(CLDSLineSlot *pLineSlot)
{
	if(pLineSlot==NULL)
		return FALSE;
	pLineSlot->BuildUCS();
	...
	{	//平铺
		...
		//转换到绘图坐标系下
		coord_trans(axis_line.startPt, cs, FALSE);
		coord_trans(axis_line.endPt, cs, FALSE);
		...
		{	//绘制脚钉图符
			...
			coord_trans(center,cs,FALSE);
			coord_trans(line1.startPt,cs,FALSE);
			coord_trans(line1.endPt,cs,FALSE);
			coord_trans(line2.startPt,cs,FALSE);
			coord_trans(line2.endPt,cs,FALSE);
	...
	{	//端面
		...
		//TODO:未完待改 wjh-2017.11.4
		if(sys.dim_map.PartNo.iPartNoFrameStyle<3)
			data_point.BuildFrame(GetPartNoIncMat(pLineSlot->GetPartNo(),pLineSlot->cMaterial),NULL,0,sys.dim_map.PartNo.iPartNoFrameStyle);
		else	//自动判断
			data_point.BuildFrame(GetPartNoIncMat(pLineSlot->GetPartNo(),pLineSlot->cMaterial),NULL,0,-1);
		data_point.dataPos=end_firs;
		int str_len=strlen(pLineSlot->GetPartNo());
...
BOOL CSectionViewDrawing::CreateSectionViewDrawing(CSectionView *pSectionView,IModel *pTower,CLDSPartGroup *pPartGroup,bool bFinalDrawing/*=true*/)	
{
	...
	if(m_pSectionView->GetViewTypeId()==CSectionView::CLS_SLAVE_VIEW)
	{
...
//</DEVELOP_PROCESS_MARK>
//返回视图最小绘图矩形区域
f2dRect CSectionViewDrawing::GetDrawRegion()
{
	f2dRect rect;
	SCOPE_STRU scope;
													 ...
													 double fDimR/*=100*/,char *dimText/*=NULL*/)
{
	...
	if(fDimR<=0)	//Pi=3.14
	{
		double half_angle=0.5*ACOS(start_vec*end_vec);
		if(half_angle<1.57)
			fDimR = TransToModelLength(ZoomToModelTextSize(sys.dim_map.fDimAngleTextSize)*0.7*4)/tan(half_angle);
	}
	//角顶点标记点
	pAngularDim->vertex=vertex;
	coord_trans(pAngularDim->vertex,cs,FALSE);
...
void CSectionViewDrawing::DimAngle()
{
	...
	{	//基准构件为钢管时的角度标注	
		...
		{	//暂不支持平放钢管图中的角度优化布局
			...
			CDimAngularManager angulars(ZoomToModelTextSize(sys.dim_map.fDimAngleTextSize));
			...
			for(CDimAngularManager::ANGULARDIM_ITEM* pDimItem=angulars.EnumFirstDimItem();pDimItem;pDimItem=angulars.EnumNextDimItem())
				AppendAngularDim(pDimItem->vTipPoint,pDimItem->vStartVec,pDimItem->vEndVec,TransToModelLength(pDimItem->fDimRadius));
			...
			{	//添加钢管基准线与指向塔心方向的角度标注项
				...
				AppendAngularDim(center,vDimStartEdgeVec,vDimEndVec);
			}
		}
		else
		{
		...
		{
			AppendDefaultAngularDimension(pAngular,pDatumLineTube,weld_road_vec,bIncludeDatumTube,pJointTube);
			/* 以下代码已完全转移至AppendDefaultAngularDimension函数中
			CLDSPlate *pStartPlate=NULL, *pEndPlate=NULL;
			CLDSLineSlot *pStartLineSlot=NULL, *pEndLineSlot=NULL;
			CLDSLineTube *pStartLineTube=NULL, *pEndLineTube=NULL;
			CLDSParamPlate *pStartParamPlate=NULL,*pEndParamPlate=NULL;	
			...
			{
				//根据钢板中心计算板的延伸方向
				f3dPoint angle_vertex;
				if(pStartPlate)
				{	
					if((bIncludeDatumTube||pJointTube)&&
						fabs(pStartPlate->ucs.axis_z*datum_line_vec)>EPS)
						continue;	//基准钢板不是钢管轴向板
					if(pStartPlate->vertex_list.GetNodeNum()<=0)
						continue;	//非法钢板
					axis_x1=pStartPlate->ucs.axis_z^datum_line_vec;
					normalize(axis_x1);
				...
				{	
					if((bIncludeDatumTube||pJointTube)&&
						fabs(pEndPlate->ucs.axis_z*datum_line_vec)>EPS)
						continue;	//基准钢板不是钢管轴向板
					if(pEndPlate->vertex_list.GetNodeNum()<=0)
						continue;	//非法钢板
					axis_x2=pEndPlate->ucs.axis_z^datum_line_vec;
		...
		CPartGroupLocatePoint *pPrevLocatePoint,CPartGroupLocatePoint *pLocatePoint)
{
	...
	//相对尺寸标注位置
	if(pPrevSizeDim&&pPrevLocatePoint&&fabs(dimDist-pPrevSizeDim->dist)<0.5)//&&pPrevLocatePoint->m_hPart==pLocatePoint->m_hPart
	{	//合并相邻重叠尺寸
		pPrevSizeDim->nHits++;
		//调整前一个标注的终端位置以及标注位置
...
void CSectionViewDrawing::DimLocatePoint(CLDSLinePart *pDatumPart)
{
	...
	f3dPoint datumVec=pDatumPart->End()-pDatumPart->Start();
	normalize(datumVec);
	f3dPoint datumOrg=pDatumPart->Start()-datumVec*pDatumPart->startOdd();
	//定位点排序
	int i=0,j=0;
...
void CSectionViewDrawing::CreatePartSketch(CSectionView::PART_ITEM* pPartItem,BOOL bCoplanar/*=TRUE*/)
{
	...
	else if(pPart->GetClassTypeId()==CLS_LINETUBE)
	{
		...
		CreatePlateDrawing((CLDSPlate*)pPart,dcs,bCoplanar,FALSE);
	else if(pPart->GetClassTypeId()==CLS_PARAMPLATE)
		CreateParamPlateDrawing((CLDSParamPlate*)pPart,dcs,bCoplanar,FALSE);
	else if(pPart->GetClassTypeId()==CLS_LINESLOT)
		CreateLineSlotDrawing((CLDSLineSlot*)pPart);
...
bool CSectionViewDrawing::IsHasIntersObj(f3dPoint rgnVert[4],CTextOnlyDrawing *pExcusiveSelfTextDim)
{
	...
	for(CTextOnlyDrawing *pPureTextDim=pureTextDimList.GetFirst();pPureTextDim;pPureTextDim=pureTextDimList.GetNext())
	{
		if(pPureTextDim==pExcusiveSelfTextDim)
			continue;
	...
	for(CSizeTextDim *pSpaceDim=spaceDimList.GetFirst();pSpaceDim;pSpaceDim=spaceDimList.GetNext())
	{
		...
		sizedim.GetDimRect(rgnTempVert,TRUE,fZoomFontCoef);
		if(IsRectInters(rgnTempVert,rgnVert))
		{
			if(bPush)
	...
	for(CSizeAngleDim *pAngular=angularDimList.GetFirst();pAngular;pAngular=angularDimList.GetNext())
	{
		...
		angledim.GetDimTextRect(rgnTempVert,fZoomFontCoef);
		if(IsRectInters(rgnTempVert,rgnVert))
		{
			if(bPush)
...
bool CSectionViewDrawing::SearchNoneIntersPos(f3dPoint rgnSrcVert[4],f3dPoint &vertSearchVec,
		double searchStep/*=5.0*/,int maxSearchStep/*=5*/,bool bEnableHoriSearch/*=true*/,
		bool bSearchVertPrefer/*=true*/,bool bDuoDirect/*=true*/)
{
	...
	int sumSearchStep=maxSearchStep*8;
...
void CSectionViewDrawing::RelayoutDrawing()
{
	//焊接方向标识文字标注(需竖直搜索)
	ClearDimItemFlag();
	...
	for(pTextDrawing=pureTextDimList.GetFirst();pTextDrawing;pTextDrawing=pureTextDimList.GetNext())
	{
		...
		pTextDrawing->GetDimRect(rgnVertArr,fZoomFontCoef);
		f3dPoint searchPos,startPos=(rgnVertArr[0]+rgnVertArr[2])*0.5;
		BOOL bPush=pureTextDimList.push_stack();
		double fTextHeight=pTextDrawing->GetHeight()*fZoomFontCoef; //按比例调整高度 
	...
	for(pTextDrawing=pureTextDimList.GetFirst();pTextDrawing;pTextDrawing=pureTextDimList.GetNext())
	{
		...
		for(int k=0;k<2;k++)
		{
			...
			//记录字体高度
			oldHeightArr[k]=textDrawingArr[k]->textHeight;
			textDrawingArr[k]->textHeight*=1.5; //放大字体高度
		}
		f3dPoint vertSearchVec(1,0,0);	//竖直搜索方向
		RotateVectorAroundVector(vertSearchVec,pTextDrawing->angle+0.5*Pi,f3dPoint(0,0,1));
		...
		for(int k=0;k<2;k++)
		{
			if(textDrawingArr[k]==NULL)
				break;
			//按输入比例得到矩形 wht 10-09-16
			textDrawingArr[k]->GetDimRect(rgnVertArr,fZoomFontCoef);
			for(int k2=0;k2<100;k2++)
			{
				f3dPoint searchPos,startPos=(rgnVertArr[0]+rgnVertArr[2])*0.5;
				if(textDrawingArr[k]->bInXWing)
					vertSearchVec*=-1.0;
				double fTextHeight=textDrawingArr[k]->GetHeight()*fZoomFontCoef; //按比例调整高度 
				bool hasIntersObj=IsHasIntersObj(rgnVertArr,textDrawingArr[k]);
				if(!hasIntersObj)
					break;
				for(int j=0;j<4;j++)
					rgnVertArr[j]+=5*vertSearchVec;
				textDrawingArr[k]->dimPos+=5*vertSearchVec;
			}
			textDrawingArr[k]->ResetPosition();
		}
		pureTextDimList.pop_stack(pushed);
		//恢复字体高度
		for(int k=0;k<2;k++)
		{
			if(textDrawingArr[k]!=NULL)
			{		//按输入比例得到矩形 wht 10-09-16
				textDrawingArr[k]->cFlag=1;
				textDrawingArr[k]->GetDimRect(rgnVertArr,fZoomFontCoef);
				for(int i=0;i<4;i++)
					usedDimRegion.VerifyVertex(rgnVertArr[i]);
				textDrawingArr[k]->textHeight=oldHeightArr[k];
			}
		}
		if(textDrawingArr[1])
	...
	for(pSizeDim=spaceDimList.GetFirst();pSizeDim;pSizeDim=spaceDimList.GetNext())
	{
		...
		pSizeDim->cFlag=1;
		//按输入比例得到矩形 wht 10-09-16
		pSizeDim->GetDimRect(rgnVertArr,TRUE,fZoomFontCoef);
		f3dPoint searchPos,startPos=(rgnVertArr[0]+rgnVertArr[2])*0.5;
	...
	{	//局部偏移尺寸标注亦分上下侧 wht 10-08-11
		...
		pSizeDim->dimEndPos*=zoomScaleCoef;
		pSizeDim->origin*=zoomScaleCoef;
		pSizeDim->origin.y+=CSizeTextDim::dimStyle.style.arrowSize;
		pSizeDim->cFlag=1;
		//按输入比例得到矩形 wht 10-09-16
		pSizeDim->GetDimRect(rgnVertArr,FALSE,fZoomFontCoef);
		for(int i=0;i<4;i++)
			usedDimRegion.VerifyVertex(rgnVertArr[i]);
	}
	//脚钉间距正下方标注
	...
	for(pSizeDim=spaceDimList.GetFirst();pSizeDim;pSizeDim=spaceDimList.GetNext())
	{
		...
		pSizeDim->origin*=zoomScaleCoef;
		pSizeDim->cFlag=1;
		pSizeDim->ResetPosition();
		double text_height=CSizeTextDim::dimStyle.textHeight;
		pSizeDim->origin.y=dimRegion.fMinY-text_height*0.5;
		pSizeDim->GetDimRect(rgnVertArr,FALSE,fZoomFontCoef);
		for(int j=0;j<4;j++)
	...
	for(CSizeAngleDim *pAngleDim=angularDimList.GetFirst();pAngleDim;pAngleDim=angularDimList.GetNext())
	{
		...
		double text_height=CSizeAngleDim::dimStyle.textHeight*fZoomFontCoef;
		double dimRectWidth=4.2*text_height;	//一律按标注角度为6个字符处理
		double sd=vertVec*startVec,ed=vertVec*endVec;
		double dimRadiusCoef=1.0;
		if(fabs(sd)>EPS&&fabs(ed)>EPS)	//非90°或180°则调整标注位置
		{
			dimRadiusCoef=sqrt(1-sd*sd)/sd+sqrt(1-ed*ed)/ed;	
			if(fabs(dimRadiusCoef)<EPS2)	//dimRadiusCoef过小会导致标注位置过远
				dimRadiusCoef=1.0;
		}
	...
	for(pSizeDim=spaceDimList.GetFirst();pSizeDim;pSizeDim=spaceDimList.GetNext())
	{
		...
		if(pSizeDim->iOrder==3)
			pSizeDim->origin.y=dimRegion.fMaxY+text_height*0.5;
	...
	for(pTextDrawing=pureTextDimList.GetFirst();pTextDrawing;pTextDrawing=pureTextDimList.GetNext())
	{
		...
		pTextDrawing->dimPos*=zoomScaleCoef;
		pTextDrawing->cFlag=1;
		pTextDrawing->ResetPosition();
		//
		double text_height=pTextDrawing->textHeight*fZoomFontCoef; 
		if(pTextDrawing->iOrder>0)
			pTextDrawing->origin.y=dimRegion.fMaxY+text_height*0.5;
		else if(pTextDrawing->iOrder<0)	//下侧绝对尺寸暂默认偏移4个字高
			pTextDrawing->origin.y=dimRegion.fMinY-text_height*4;
	...
	for(pTextFrame=dataPointList.GetFirst();pTextFrame;pTextFrame=dataPointList.GetNext())
	{
		...
		double fHeight=pTextFrame->Height();//*fZoomFontCoef;
		if(SearchNoneIntersPos(rgnVertArr,f3dPoint(0,1,0),fHeight/2,20*ftoi(fZoomFontCoef),true,false))
		{
			searchPos=(rgnVertArr[0]+rgnVertArr[2])*0.5;
	...
	{
		pTextDrawing=pureTextDimList.append();
		pTextDrawing->angle=0;		//旋转角
		pTextDrawing->bNeedConvertDistToText=false;
		pTextDrawing->iOrder=2;		//视图名称
		pTextDrawing->alignType=MiddleCenter;
		pTextDrawing->textHeight=1.2*sys.dim_map.fDimTextSize;	//暂定剖视图名称为1.2倍普通标注字高
		//按比例调整字体高度 wht 10-09-14
		double text_height=pTextDrawing->GetHeight()*fZoomFontCoef;
		pTextDrawing->dimPos.Set((dimRegion.fMaxX+dimRegion.fMinX)*0.5,dimRegion.fMinY-text_height);
		pTextDrawing->dimText.Printf("%s-%s",(char*)m_pSectionView->Symbol,(char*)m_pSectionView->Symbol);
		//剖视图名称下的横线
		GEPOINT lineStart(pTextDrawing->dimPos.x-text_height*2,pTextDrawing->dimPos.y-text_height);
		GEPOINT lineEnd(pTextDrawing->dimPos.x+text_height*2,pTextDrawing->dimPos.y-text_height);
		...
		pPolyline->SetLayer(LayerTable::VisibleProfileLayer.layerName);
		pPolyline->AddVertexAt(0,lineStart,1,1);
		pPolyline->AddVertexAt(1,lineEnd,0,0);
		dimRegion.VerifyVertex(lineStart);
		dimRegion.VerifyVertex(lineEnd);

		lineStart.y=lineEnd.y=pTextDrawing->dimPos.y-text_height-1;
		IDbLine* pLine=(IDbLine*)AppendDbEntity(IDbEntity::DbLine);
		pLine->SetLayer(LayerTable::VisibleProfileLayer.layerName);
...
void CSectionViewDrawing::CreateTubeFeatureLine(SCOPE_STRU& region)
{
	...
	{	//手动绘制箭头模式
		...
		orthoLineStart=GetFeatureVecPoint(arrowOffsetVec,region);
		orthoLineEnd  =GetFeatureVecPoint(arrowOffsetVec,region,FALSE);
	...
	if(!tube_base_line.m_xTaCoreVec.IsZero())
	{	//绘制朝向塔心的示意图
		f3dPoint xRootHead=GetFeatureVecPoint(tube_base_line.m_xTaCoreVecHead,region);
		...
		f3dPoint uppervec(-tube_base_line.m_xTaCoreVec.y,tube_base_line.m_xTaCoreVec.x);
		//需要处理字体高度与绘图比例随动的情况　2017.11.8
		f3dPoint dimpos=xRoot-tube_base_line.m_xTaCoreVec*ZoomToModelTextSize(sys.fonts.weld.fCutawayViewSymbolTextSize)*2+uppervec*0.5;
...
{
	CLDSLineTube* pTube=(CLDSLineTube*)m_pRootPart;
	if(m_pSectionView->GetViewTypeId()==CSectionView::CLS_SLAVE_NODESECT_VIEW)
		pTube=(CLDSLineTube*)Ta.FromPartHandle(((CSlaveNodeSectionView*)m_pSectionView)->m_hVertRod,CLS_LINETUBE);
	if(pTube==NULL)
		return;
	PEN_STRU penStyle;
	penStyle.width=0;
	...
	for(int i=0;i<rayVecs.GetSize();i++)
	{
		...
		if(fabs(ray_vec*base_vec)>EPS_COS)	//标注射线与基准线重合
			continue;
		GEPOINT intersPt=GetFeatureVecPoint(ray_vec,region);
		intersPt+=ray_vec*1;
		penStyle.style=PS_DASHDOTDOT;
		IDbLine* pLine=(IDbLine*)m_xEntManager.AppendDbEntity(IDbEntity::DbLine);
...
{	//由于应该函数位于RelayoutDrawing()末尾调用，未处理比例尺问题，故需要在该函数内部手动处理绘图比例 wjh-2017.11.21
	...
	{	//上侧卡板
		...
		GEPOINT xArcPoint(center+midDirection*(pDbText->GetHeight()*1.5+(radius+50)/m_fUser2ScrDrawScale));
		if(midDirection.y<0)	//原本文字头朝向，掉转180度后应再多移一个字高对齐 wjh-2017.11.21
...
void CSectionStdXPlatesDrawing::DimViewHorizontalLsSpace(CDrawCoordTrans &dcs,BOOL bFrontView,f3dPoint plate_pos)
{	//1.统计螺栓位置并X坐标排序
	ATOM_LIST<double> dimPosList;
	CLDSPlate* platePtrArr[2]={m_xParam.m_pBasePlate,NULL};
	double fPlateHalfThick=m_xParam.m_pFrontVertPlate->GetThick()*0.5;
	if(!bFrontView)
	{
		fPlateHalfThick=m_xParam.m_pBasePlate->GetThick()*0.5;
		platePtrArr[0]=m_xParam.m_pFrontVertPlate;
		platePtrArr[1]=m_xParam.m_pBackVertPlate;
	}
	dimPosList.append(plate_pos.x-fPlateHalfThick);
	dimPosList.append(plate_pos.x+fPlateHalfThick);
	for(int i=0;i<2;i++)
	{
		...
		for(CLsRef *pLsRef=platePtrArr[i]->GetFirstLsRef();pLsRef;pLsRef=platePtrArr[i]->GetNextLsRef())
		{
			...
			double *pValue=NULL;
			for(pValue=dimPosList.GetFirst();pValue;pValue=dimPosList.GetNext())
			{
				if(fabs(ls_pos.x-*pValue)<2)
					break;
				if(ls_pos.x<*pValue)
				{
					dimPosList.insert(ls_pos.x,dimPosList.GetCurrentIndex());
					break;
				}
			}
			if(pValue==NULL)
				dimPosList.append(ls_pos.x);
		}
	}
	//2.标注螺栓间距
	f2dRect rect=GetDrawRegion();
	double fOffsetDist=TransToModelLength(CSizeTextDim::dimStyle.textHeight);
	for(int i=1;i<dimPosList.GetNodeNum();i++)
	{
		double *pPrevValue=dimPosList.GetByIndex(i-1);
		double *pValue=dimPosList.GetByIndex(i);
		f3dPoint dimStart(*pPrevValue,rect.bottomRight.y-10,0),dimEnd(*pValue,dimStart.y,0);
		f3dPoint dimPos(0.5*(*pPrevValue+*pValue),dimStart.y-fOffsetDist,0);
		double fDist=*pValue-*pPrevValue;
		if( fabs(*pPrevValue-(plate_pos.x-fPlateHalfThick))<EPS&&
			fabs(*pValue-(plate_pos.x+fPlateHalfThick))<EPS)
			AppendSizeTextDim(dimStart,dimEnd,dimPos,fDist,0,FALSE,TRUE);
		else 
...
BOOL CSectionAngleFootDrawing::CreateSectionViewDrawing(CSectionView* pSectionView,IModel* pTower,CLDSPartGroup* pPartGroup,bool bFinalDrawing/*=true*/)
{
	...
	if(pAngleFootView->m_cSubType=='F')
		...
		CreateSideViewDrawing();
	else if(pAngleFootView->m_cSubType=='D'||pAngleFootView->m_cSubType=='U')
		CreateTopViewDrawing();
	else 
		return FALSE;
	//3、对视图进行布局
...
void CFLDrawing::DrawFLStruDrawing(CLDSParamPlate* pPlate,CDrawCoordTrans& dcs,IDbEntsContext* pContext)
{
	...
	datapoint.InitTextFrame(pContext->ZoomToModelTextSize(sys.dim_map.PartNo.fPartNoTextSize),pContext->ZoomToModelTextSize(sys.dim_map.PartNo.fPartNoCirD),-1,pContext->ZoomToModelTextSize(sys.dim_map.PartNo.fPartNoMargin));
	...
	pContext->AppendDataPoint(datapoint);
	//标注法兰外径
	double fOffsetDist=pContext->TransToModelLength(pContext->ZoomToModelTextSize(CSizeTextDim::dimStyle.textHeight));
...
void CPartGroupTblDrawing::CreatePartGroupBom(double scale)
{
	...
	for(i=0;i<nRowsN;i++)			//数据行
		m_xPartTbl.SetRowHigh(i,4*scale);
	//计算列数设置列宽以及表格宽度
	//大吉熊信贵说：尺寸可否固定行间距均为4，宽度从左到右依次为15，6，4，4 增加两列是焊缝级别，我们自己来标 wjh-2013.12.06
	if(sys.part_group_map.part_table.bDispPartGroupNo)
	{
		m_xPartTbl.SetColumnWide(iCurCol,9*scale);	//组焊编号栏
		iCurCol++;
	}
	if(sys.part_group_map.part_table.bDispPartNo)
	{
		m_xPartTbl.SetColumnWide(iCurCol,15*scale);	//单个构件编号栏
		iCurCol++;
	}
	if(sys.part_group_map.part_table.bDispPartGuiGe)
	{
		m_xPartTbl.SetColumnWide(iCurCol,27*scale);	//规格栏//原为25但由于--Q345有些规格放不下,故放大为27 WJH--01.11.01
		iCurCol++;
	}
	if(sys.part_group_map.part_table.bDispPartLen)
	{
		m_xPartTbl.SetColumnWide(iCurCol,15*scale);	//长度栏
		iCurCol++;
	}
	if(sys.part_group_map.part_table.bDispPartNum)
	{
		m_xPartTbl.SetColumnWide(iCurCol,6*scale);	//数量栏
		iCurCol++;
	}
	if(sys.part_group_map.part_table.bDispPartWeight)
	{
		m_xPartTbl.SetColumnWide(iCurCol,15*scale);	//重量之一件栏
		iCurCol++;
		m_xPartTbl.SetColumnWide(iCurCol,15*scale);	//重量之小计栏
		iCurCol++;
	}
	if(sys.part_group_map.part_table.bDispNotes)
	{
		m_xPartTbl.SetColumnWide(iCurCol,20*scale);	//备注栏
		iCurCol++;
	}
	if(sys.part_group_map.part_table.bDispPartGroupNum)
	{
		m_xPartTbl.SetColumnWide(iCurCol,9*scale);	//组合构件单基数量
		iCurCol++;
	};
	//大吉熊信贵说：尺寸可否固定行间距均为4，宽度从左到右依次为15，6，4，4 增加两列是焊缝级别，我们自己来标 wjh-2013.12.06
	if(sys.part_group_map.part_table.bDispInternal)
	{
		m_xPartTbl.SetColumnWide(iCurCol,4*scale);	//焊缝级别－内在
		iCurCol++;
	}
	if(sys.part_group_map.part_table.bDispAppearance)
	{
		m_xPartTbl.SetColumnWide(iCurCol,4*scale);	//焊缝级别－外观
		iCurCol++;
	}
	//绘制表格
	...
	//定义--单个构件编号表格单元
	iCurCol=0;
	int iStartRow=1,iEndRow=nRowsN;	//开始行结束行
	...
	{//组焊编号中不显示材质字符 wjh-2013.5.20
		m_xPartTbl.GetGridAt(iStartRow,0)->data.SetGridString(m_pPartGroup->sGroupName);
	...
	{	//定义合计行
		...
		m_xPartTbl.GetGridAt(nRowsN-1,iStartCol+2)->data.SetGridString(tem_str);
		m_xPartTbl.GetGridAt(nRowsN-1,iStartCol+2)->feature = 1;	//文字中央对齐
	}
...
void CPartGroupTblDrawing::CreateSphereIntersTbl(double scale)
{
	...
	for(i=0;i<nRowsN;i++)			//数据行
		m_xPartTbl.SetRowHigh(i,4*scale);
	//计算列数设置列宽以及表格宽度
	m_xPartTbl.SetColumnWide(0,11*scale);	//钢管编号栏
	m_xPartTbl.SetColumnWide(1,15*scale);	//所属视图编号栏
	m_xPartTbl.SetColumnWide(2,20*scale);	//球心到法兰外端面距离
	m_xPartTbl.SetColumnWide(3,10*scale);	//球体上装配点序号
	m_xPartTbl.SetColumnWide(4,37*scale);	//球体上装配点信息栏
	//绘制表格
	int iCurCol=0;
	m_xPartTbl.GetGridAt(1,iCurCol)->m_bVGridVirtual = TRUE;//坚直合并单元格
#ifdef AFX_TARG_ENU_ENGLISH
...
f2dRect CPartGroupTblDrawing::GetDrawRegion()
{
	f2dRect rect;
	rect.topLeft.Set(0,0);
...
f2dRect CSectionKmAngleViewDrawing::GetDrawRegion()
{
	double zoomScaleCoef=1.0/m_fUser2ScrDrawScale;
	f2dRect rect;
	rect.topLeft.Set(0,0);
...
f2dRect CSectionAngleViewDrawing::GetDrawRegion()
{
	double zoomScaleCoef=1.0/m_fUser2ScrDrawScale;
	...
	xAngleDrawing.SetVertScaleCoef(zoomScaleCoef);
	xAngleDrawing.SetLengthScaleCoef(zoomScaleCoef);
	xAngleDrawing.CreateLineAngleDrawing(pLineAngle,pLineAngle->GetLength(),pLineAngle->GetWidth()*2);
	return xAngleDrawing.GetDrawRegion(1.0);
}
...
void CJointTubeDimDrawingCommand::CreateJointTubeDims()
{
	...
	double fTextHight=CSizeTextDim::dimStyle.textHeight*20;
...
//根据短角钢提取接头处的关联构件
void CJointWrapperView::ExtractRelaParts()
{
	if(m_pShortJg==NULL)
		return;
	//查找始终端特征螺栓，提取依赖角钢及包板
	f3dPoint normX=m_pShortJg->vxWingNorm,normY=m_pShortJg->vyWingNorm;
	double fMinXS=10000,fMinXE=10000,fMinYS=10000,fMinYE=10000;;
	CLDSBolt *pXBoltS=NULL,*pXBoltE=NULL,*pYBoltS=NULL,*pYBoltE=NULL;
	for(CLsRef* pLsRef=m_pShortJg->GetFirstLsRef();pLsRef;pLsRef=m_pShortJg->GetNextLsRef())
	{
		CLDSBolt* pBolt=pLsRef->GetLsPtr();
		double dds=DISTANCE(pBolt->ucs.origin,m_pShortJg->Start());
		double dde=DISTANCE(pBolt->ucs.origin,m_pShortJg->End());
	...
	{	//提取短角钢始端关联构件
		...
		for(THICK_ITEM item=pBolt->GetL0ThickPara()->GetFirstItem();item.val!=0;item=pBolt->GetL0ThickPara()->GetNextItem())
		{
			...
			if(pPart==NULL)
				continue;
			if(m_pDependJgS && m_pDependJgS->handle==pPart->handle)
				continue;
			if(m_pDependJgS==NULL && pPart->GetClassTypeId()==CLS_LINEANGLE)
				m_pDependJgS=(CLDSLineAngle*)pPart;
			CLDSPlate* pPlate=NULL;
			if(pPart->GetClassTypeId()==CLS_PLATE)
				pPlate=(CLDSPlate*)pPart;
			if(i==0 && pPlate)
			{
				if(m_pXWrapPlate==NULL)
					m_pXWrapPlate=(CLDSPlate*)pPart;
				else if(m_pXPadPlate==NULL)
					m_pXPadPlate=(CLDSPlate*)pPart;
			}
			else if(i==1 && pPlate)
			{
				if(m_pYWrapPlate==NULL)
					m_pYWrapPlate=(CLDSPlate*)pPart;
				else if(m_pYPadPlate==NULL)
					m_pYPadPlate=(CLDSPlate*)pPart;
			}
		}
	}
	if(m_pXPadPlate&&m_pYPadPlate)
		m_bStartHasPad=TRUE;
	for(int i=0;i<2;i++)
	{	//提取短角钢终端关联构件
		...
		for(THICK_ITEM item=pBolt->GetL0ThickPara()->GetFirstItem();item.val!=0;item=pBolt->GetL0ThickPara()->GetNextItem())
		{
			...
			if(pPart==NULL)
				continue;
			if(m_pDependJgE && m_pDependJgE->handle==pPart->handle)
				continue;
			if(m_pDependJgE==NULL && pPart->GetClassTypeId()==CLS_LINEANGLE)
				m_pDependJgE=(CLDSLineAngle*)pPart;
			CLDSPlate* pPlate=NULL;
			if(pPart->GetClassTypeId()==CLS_PLATE)
				pPlate=(CLDSPlate*)pPart;
			if(pPlate && !m_bStartHasPad)
			{	//始端无垫板，判断终端是否有垫板
				if(i==0&&pPlate->handle!=m_pXWrapPlate->handle)
					m_pXPadPlate=pPlate;
				if(i==1&&pPlate->handle!=m_pYWrapPlate->handle)
					m_pYPadPlate=pPlate;
			}
			else if(pPlate && m_bStartHasPad)
			{	//始端有垫板，根据终端螺栓确定正确的包板和垫板
				if(i==0&&pPlate->handle==m_pXPadPlate->handle)
				{
					m_pXPadPlate=m_pXWrapPlate;
					m_pXWrapPlate=pPlate;
				}
				if(i==1&&pPlate->handle==m_pYPadPlate->handle)
				{
					m_pYPadPlate=m_pYWrapPlate;
					m_pYWrapPlate=pPlate;
				}
			}
		}
	}
	if(m_pXWrapPlate&&m_pYPadPlate&&m_bStartHasPad==FALSE)
		m_bEndHasPad=TRUE;
}
void CJointWrapperView::BuildDrawCs()
{
	...
	ucs=cs.GetACS();
	//计算角钢肢偏移方向
	f3dPoint wingX_vec=m_pShortJg->GetWingVecX();
	f3dPoint wingY_vec=m_pShortJg->GetWingVecY();
...
//判断是否为内包角钢
BOOL CJointWrapperView::IsInnerWrapperJg()
{
	if(m_pShortJg==NULL || m_pDependJgS==NULL)
		return FALSE;
	f3dPoint datum_ber_pt=m_pDependJgS->Start();
	f3dPoint ber_pt=m_pShortJg->Start();
	coord_trans(datum_ber_pt,ucs,FALSE);
	coord_trans(ber_pt,ucs,FALSE);
	if(datum_ber_pt.x<ber_pt.x)
		return TRUE;
	else
		return FALSE;
}
//计算偏移值
int CJointWrapperView::CalOffDist(CLDSLineAngle* pJg/*=NULL*/)
{
	int nOffDist=0;
	if(pJg==NULL)
	{
		...
		if(!m_bStartHasPad)
			pDependJg=m_pDependJgS;
		else if(!m_bEndHasPad)
			pDependJg=m_pDependJgE;
		if(pDependJg==NULL)
			return nOffDist;
		...
		nOffDist=ftoi(fabs(ber_pt.x)-thick);
	}
	else
	{
		f3dPoint ber_pt=pJg->Start();
		coord_trans(ber_pt,ucs,FALSE);
		ber_pt.z;
		double thick=m_pShortJg->Thick;
		if(IsInnerWrapperJg())
			thick=pJg->Thick;
		nOffDist=ftoi(fabs(ber_pt.x)-m_nGap);
	}
	return nOffDist;
}
void CJointWrapperView::CreateDrawing()
{
	if(m_pDependJgS==NULL || m_pDependJgE==NULL)
		ExtractRelaParts();
	m_pShortJg->BuildUCS();
	BuildDrawCs();
	m_nGap=CalOffDist();	//计算相邻构件的间隙值
	f3dPoint orign;
	for(int i=0;i<2;i++)
	{
		CLDSLineAngle* pDependJg=NULL;
		...
		{
			orign.Set(3*m_pShortJg->GetWidth(),0,0);
			pDependJg=m_pDependJgE;
		}
		//绘制角钢截面
		DrawLineAngleSection(m_pShortJg,orign);
		DrawLineAngleSection(pDependJg,orign);
		//绘制钢板截面
		DrawPlateSection(m_pXWrapPlate,orign);
		DrawPlateSection(m_pYWrapPlate,orign);
		if((i==0&&m_bStartHasPad)||(i==1&&m_bEndHasPad))
		{
			DrawPlateSection(m_pXPadPlate,orign,TRUE);
			DrawPlateSection(m_pYPadPlate,orign,TRUE);	
		}
		//标注角钢螺栓
		DimAngleLsSpace(pDependJg,orign);
	}
}
void CJointWrapperView::DrawLineAngleSection(CLDSLineAngle* pAngle,f3dPoint base_pnt)
{
	BOOL bDependJg=FALSE;
	if(m_pShortJg->handle!=pAngle->handle)
		bDependJg=TRUE;
	BOOL bInnerWrapper=IsInnerWrapperJg();		//是否为内包铁
	...
	{
		int nOffDist=CalOffDist(pAngle);
		if(bInnerWrapper)	//内包
	...
	line.ID=0;
	//
	if((bInnerWrapper&&bDependJg==FALSE)||(!bInnerWrapper&&bDependJg))
	{//内包铁外楞需圆弧过渡
		datumPt3=ber_pt;
		...
		center_pick=f3dPoint(arc_end.x,arc_start.y)+base_pnt;
	...
	center_pick=f3dPoint(arc_end.x+y_off_vec*thick,arc_end.y);
...
void CJointWrapperView::DimAngleLsSpace(CLDSLineAngle* pAngle,f3dPoint base_pnt)
{
	if(m_pShortJg->handle==pAngle->handle)
		return;	//短角钢不处理螺栓标注
	...
	{
		datumPt=datumPt1;
		fDist=m_pXWrapPlate?m_pXWrapPlate->Thick:2;
	}
	else
	{	
		datumPt=datumPt3;
		fDist=m_pShortJg->Thick;
		fDist+=m_pXPadPlate?m_pXPadPlate->Thick:2;
	}
	fDist+=pAngle->Thick;
	//绘制螺栓线
	f3dLine line;
