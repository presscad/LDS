//<LOCALE_TRANSLATE/>
// DrawUcsDlg.cpp : implementation file
//
...
CDrawUcsDlg::CDrawUcsDlg(CWnd* pParent /*=NULL*/)
	: CDialog(CDrawUcsDlg::IDD, pParent)
{
	//{{AFX_DATA_INIT(CDrawUcsDlg)
	m_fAxisX_X = 1.0;
	m_fAxisX_Y = 0.0;
	m_fAxisX_Z = 0.0;
	m_fAxisY_X = 0.0;
...
void CDrawUcsDlg::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CDrawUcsDlg)
...
BEGIN_MESSAGE_MAP(CDrawUcsDlg, CDialog)
	//{{AFX_MSG_MAP(CDrawUcsDlg)
...
/////////////////////////////////////////////////////////////////////////////
// CDrawUcsDlg message handlers

BOOL CDrawUcsDlg::OnInitDialog() 
{
...
void CDrawUcsDlg::OnBnPasteAxisX() 
{
	f3dPoint point;
...
void CDrawUcsDlg::OnBnPasteAxisY() 
{
	f3dPoint point;
	if(ReadClipPoint(point))
	{
...
void CDrawUcsDlg::OnBnPasteAxisZ() 
{
	f3dPoint point;
	if(ReadClipPoint(point))
	{
...
void CDrawUcsDlg::OnBnPasteOriginPos() 
{
	f3dPoint point;
	if(ReadClipPoint(point))
	{
...
void CDrawUcsDlg::OnOK() 
{
	UpdateUcs();
	if(ucs.axis_x.IsZero()||ucs.axis_y.IsZero()||ucs.axis_z.IsZero())
#ifdef AFX_TARG_ENU_ENGLISH
		AfxMessageBox("coordinate is illegal(axis direction cannot be zero)");
	else if(!is_ucs_valid(ucs))
		AfxMessageBox("coordinate is illegal(coordinate is right hand spiral orthogonal)");
#else
...
void CDrawUcsDlg::UpdateUcs(BOOL bFlag)
{
	if(bFlag)	//交互界面向ucs更新
	{
		UpdateData(TRUE);
...
void CDrawUcsDlg::OnBnCopyAxisX() 
{
	f3dPoint point;
...
void CDrawUcsDlg::OnBnCopyAxisY() 
{
	f3dPoint point;
...
void CDrawUcsDlg::OnBnCopyAxisZ() 
{
	f3dPoint point;
...
void CDrawUcsDlg::OnBnCopyOriginPos() 
{
	f3dPoint point;
...
void CDrawUcsDlg::OnBnToRightHand() 
{
	UpdateUcs();
...
void CDrawUcsDlg::OnChkEnableAxisX() 
{
	UpdateData();
...
void CDrawUcsDlg::OnChkEnableAxisY() 
{
	UpdateData();
...
void CDrawUcsDlg::OnChkEnableAxisZ() 
{
	UpdateData();
	if(!m_bCanModifyAxisZ)
...
HBRUSH CDrawUcsDlg::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor) 
{
